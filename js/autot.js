...
function autotrackSetStatus(text) {
  document.getElementById("autotrackingstatus").textContent =
    "Autotracking Status: " + text;
}
...
function autotrackDoTracking(data) {
  ...
  function newbit_group(locations) {
    ...
    for (const location of locations) {
      if ((data[location[0]] & location[1]) === 0) return false;
      if (
        autotrackPrevData === null ||
        (autotrackPrevData[location[0]] & location[1]) === 0
      )
        activated = true;
    }
    return activated;
  ...
  function updatechest(chest, offset, mask) {
    if (newbit(offset, mask) && !chests[chest].is_opened) toggle_chest(chest);
  }
    ...
    if (changed(offset)) {
      var newkeys =
        autotrackPrevData === null
          ? data[offset]
          : data[offset] -
            autotrackPrevData[offset] +
            items["smallkey" + dungeon];
  ...
  if (newbit(0x38e, 0xc0)) {
    ...
    setitem(
      "bow",
      bits == 0x40 && flags.nonprogressivebows ? 1 : bits == 0x80 ? 2 : 3,
    );
  }
