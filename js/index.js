...
export function toggle(x, y) {
  document
    .getElementById("starting" + x)
    .classList.remove(x + startingitemstring.charAt(y));
  switch (y) {
    case 1:
    case 2:
      startingitemstring =
        startingitemstring.substring(0, y) +
        (startingitemstring.charAt(y) === "0"
          ? "1"
          : startingitemstring.charAt(y) === "1"
            ? "2"
            : startingitemstring.charAt(y) === "2"
              ? "3"
              : "0") +
        startingitemstring.substring(y + 1);
      break;
    case 23:
      startingitemstring =
        startingitemstring.substring(0, y) +
    ...
    default:
      startingitemstring =
        startingitemstring.substring(0, y) +
        (startingitemstring.charAt(y) === "0" ? "1" : "0") +
        startingitemstring.substring(y + 1);
      break;
  }
  document
    .getElementById("starting" + x)
    .classList.add(x + startingitemstring.charAt(y));
  document.getElementById("starting" + x).style.opacity =
    startingitemstring.charAt(y) === "0" ? "0.25" : "1.0";
}
...
export function launch_tracker() {
  var world = document.querySelector(
    'input[name="gametypegroup"]:checked',
  ).value;
  var entrance = document.querySelector(
    'input[name="entrancegroup"]:checked',
  ).value;
  var door = document.querySelector('input[name="doorgroup"]:checked').value;
  var overworld = document.querySelector(
    'input[name="overworldgroup"]:checked',
  ).value;
  ...
  var unknown = document.querySelector(
    'input[name="unknowngroup"]:checked',
  ).value;
  var glitches = document.querySelector(
    'input[name="glitchesgroup"]:checked',
  ).value;
  var shuffledmaps =
    document.getElementById("shuffledmaps").checked === true ? "1" : "0";
  var shuffledcompasses =
    document.getElementById("shuffledcompasses").checked === true ? "1" : "0";
  var shuffledsmallkeys =
    document.getElementById("shuffledsmallkeys").checked === true ? "1" : "0";
  var shuffledbigkeys =
    document.getElementById("shuffledbigkeys").checked === true ? "1" : "0";
  var shopsanity = document.querySelector(
    'input[name="shopsanitygroup"]:checked',
  ).value;
  var ambrosia = document.querySelector(
    'input[name="ambrosiagroup"]:checked',
  ).value;
  var nonprogressivebows = document.querySelector(
    'input[name="nonprogressivebowsgroup"]:checked',
  ).value;
  var activatedflute = document.querySelector(
    'input[name="activatedflutegroup"]:checked',
  ).value;
  ...
  var swords = document.querySelector(
    'input[name="swordsgroup"]:checked',
  ).value;
  var map = document.querySelector('input[name="mapgroup"]:checked').value;
  var spoiler = document.querySelector(
    'input[name="spoilergroup"]:checked',
  ).value;
  var sphere = document.querySelector(
    'input[name="spheregroup"]:checked',
  ).value;
  var color = document.querySelector(
    'input[name="alternatecolorgroup"]:checked',
  ).value;
  var autotracking = document.querySelector(
    'input[name="autotrackinggroup"]:checked',
  ).value;
  ...
  var restreamer = document.querySelector(
    'input[name="restreamgroup"]:checked',
  ).value;
  ...
  var height =
    sphere === "Y" ? (map === "C" ? 988 : 744) : map === "C" ? 692 : 448;
  ...
  if (document.getElementById("remembersettings").checked == true) {
    var settings =
      ...
      sprite;
    document.cookie =
      "settings=" + settings + "; expires=Sat, 3 Jan 2026 12:00:00 UTC";
  } else {
  ...
  if (glitches === "O" && world === "I") {
    alert(
      "NOTICE: Inverted OWG is currently not supported for logic, all locations will be flagged as available.",
    );
    glitches = "M";
  }
...
export function importflags() {
  ...
  $.getJSON(
    "https://alttpr-patch-data.s3.us-east-2.amazonaws.com/" + i + ".json",
    function (data) {
      var d = data.spoiler;
      ...
      if (d.meta.spoilers === "mystery") {
        ...
        if (d.meta.entry_crystals_tower === "random") {
          ...
          document.getElementById("goalcrystal").checked = true;
          document.getElementById("towerselect").value =
            d.meta.entry_crystals_tower;
        }
        ...
        if (d.meta.entry_crystals_ganon === "random") {
          ...
          document.getElementById("ganoncrystal").checked = true;
          document.getElementById("ganonselect").value =
            d.meta.entry_crystals_ganon;
        }
...
export function validateRestreamCode() {
  ...
  xhr.open(
    "GET",
    "https://alttprtracker.dunka.net/api/v1/RestreamerAPI/ValidateCode?code=" +
      document.getElementById("restreamingcode").value,
    true,
  );
