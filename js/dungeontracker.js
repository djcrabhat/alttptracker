(function (window) {
  ...
  const searchStartDefaultHelp = {
    canstartcommon:
      "Takes both the selected screen and all common start points (save & quit spots) into account, mixed together.",
    onlystartscreen:
      "Always starts at selected screen, but can save & quit to common starts and use flute (if available).",
    noflutequit:
      "Always starts at selected screen and searches direct paths without using save & quit or flute.",
  };
  const importantRoomNodesHelp = {
    fewer:
      'Similar to "Default", but some of the simpler rooms are left out (mostly from groups of multiple similar rooms).',
    default:
      "Roughly, these are the rooms or supertiles that are bounded by at least 5 screen transitions, or that play a special role (like TT Attic).",
  ...
  window.loadOverview = function () {
    ...
    if (
      (!welcomeMode && document.getElementById("itemsync").checked) ||
      document.getElementById("connectorsync").checked ||
      document.getElementById("globalsync").checked
    )
      testConnection(true);
  };
  ...
  window.loadDungeonSummary = function (n) {
    if (dungeonPaths[n].completed) {
      ...
      document.getElementById("summary" + n).innerHTML =
        "<div class='symbolnode' style='background-image: url(" +
        "./images/dungeons/keychest0.png); left: 16px;'></div>";
      if (visibleSidebar)
        document.getElementById("sidesummary" + n).innerHTML =
          "<div class='symbolnode' style='background-image: url(" +
          "./images/dungeons/keychest0.png);'></div>";
    } else {
      ...
      for (let path of dungeonPaths[n].paths) {
        for (let l = 0; l < path.length - 1; l += 2) {
          if (path[l] === "i" || path[l] === "b") {
            ...
            if (
              (excludesmallkeys && id === "i0") ||
              (excludebigkeys && id === "i1")
            )
              continue;
  ...
  window.loadDungeon = function (n) {
    currentDungeon = n;
    document.getElementById("dungeonicon").className =
      "boss boss" + n + (dungeonPaths[n].completed ? " completed" : "");
    document.getElementById("dungeonname").innerHTML = dungeonNamesLong[n];
    updateStartRooms();
    document.getElementById("completedbutton").innerHTML = dungeonPaths[n]
      .completed
      ? "Dungeon not completed"
  ...
  window.createImportantRoomNodeList = function (type, clickAction) {
    let minPriority = { fewer: 3, default: 2, more: 1, nondefault: 1 }[
      type ? type : importantRoomNodes
    ];
    ...
    for (let k = 0; k < dungeonImportant.length; k++)
      if (
        all
          ? dungeonImportant[k].priority >= minPriority &&
            dungeonImportant[k].priority <= maxPriority
          : dungeonImportant[k].dungeon === currentDungeon
      )
  ...
  window.updateStartRooms = function () {
    document.getElementById("dungeonentrances").innerHTML = createEntranceNodes(
      false,
      "startPath",
    );
    document.getElementById("dungeonimportant").innerHTML =
      document.getElementById("showmorerooms").checked
        ? createImportantRoomNodeList(null, "startPath")
        : "";
    document.getElementById("dungeonused").innerHTML =
      createUsedRoomNodeList("startPath");
  };
  ...
  window.markCompleted = function () {
    if (dungeonPaths[currentDungeon].completed) {
      ...
      document.getElementById("completedbutton").innerHTML =
        "Mark dungeon completed";
  ...
  window.createRoomNode = function (
    room,
    showDungeonName = false,
    clickAction = "startPath",
    left = 0,
    top = 0,
  ) {
    let file,
    ...
    if (room.file) {
      ...
      scale = 0.5;
      name = showDungeonName
        ? dungeonNamesShort[room.dungeon] + " " + room.name
        : room.name;
      switch (room.part) {
        case "topleft":
          scale = 1;
    ...
    let onClick =
      clickAction === "startPath"
        ? 'startPath("' + room.id + '")'
        : clickAction === "append"
          ? 'appendToPath("' + room.id + '"); hideRoomModal()'
          : "";
    return (
      "<div class='" +
      classes +
  ...
  window.createSymbolNode = function (
    symbol,
    left,
    top,
    small = false,
  ...
  window.drawPath = function (path, markLastBranch = false, maxWidth = 0) {
    ...
    if (markLastBranch && lastBranchLeft != -1) {
      ...
      s +=
        "<div class='symbolnode' style='width: 16px; height: 64px; border-left: 2px dashed " +
        (theme === "light" ? "#C0C000" : "yellow") +
        "; opacity: .75; left: " +
        lastBranchLeft +
        "px; top: " +
        lastBranchTop +
  ...
  window.moveToTop = function () {
    ...
    if (currentPath != null && currentlyEditing) {
      ...
      document
        .getElementById("dungeonpaths2")
        .childNodes[editingIndex].classList.add("editing");
    }
    document
      .getElementById(currentPath === null ? "dungeonpaths" : "dungeonpaths2")
      .childNodes[0].classList.add("newpath");
    loadDungeonSummary(currentDungeon);
    hideClickPathModal();
    sendUpdate();
  };
  ...
  window.moveToBottom = function () {
    ...
    document
      .getElementById(currentPath === null ? "dungeonpaths" : "dungeonpaths2")
      .childNodes[
        dungeonPaths[currentDungeon].paths.length - 1
  ...
  window.startPath = function (path, editing = false) {
    currentPath = path;
    document.getElementById("currentpathheader").innerHTML =
      (editing ? "Editing an existing" : "Creating a new") +
      " path in " +
      dungeonNamesShort[currentDungeon];
  ...
  window.appendToPath = function (id) {
    ...
    if (
      ...
      document.getElementById("splitpath").checked
    ) {
      ...
      document
        .getElementById("dungeonpaths2")
        .childNodes[indexNew].classList.add("newpath");
    } else {
      document.getElementById("currentpath").innerHTML = drawPath(
        currentPath,
        true,
      );
      document.getElementById("deletebutton").classList.remove("disabled");
  ...
  window.savePath = function () {
    ...
    updatePathList();
    document
      .getElementById("dungeonpaths")
      .childNodes[currentlyEditing ? editingIndex : 0].classList.add("newpath");
    switchScene("pathlist");
    currentPath = null;
    loadDungeonSummary(currentDungeon);
    sendUpdate();
  ...
  window.updateStatistics = function () {
    ...
    document.getElementById("totalpathdungeons").innerHTML =
      "Dungeons with paths: " + dungeonPathCount;
    document.getElementById("totalpaths").innerHTML =
      "Total paths: " + pathCount;
    document.getElementById("totalnodes").innerHTML =
      "Total nodes in paths: " + nodeCount;
    document.getElementById("totalnotes").innerHTML =
      "Dungeons with notes: " + noteCount;
  };

  window.updatePathList = function () {
    document.getElementById("dungeonpathsheader").innerHTML =
      "Your paths (" + dungeonPaths[currentDungeon].paths.length + ")";
    document.getElementById("dungeonpaths2header").innerHTML =
      "Existing paths (" + dungeonPaths[currentDungeon].paths.length + ")";
    let s = "";
    for (let k = 0; k < dungeonPaths[currentDungeon].paths.length; k++)
      s +=
        '<div class="row path" style="margin: 2px 0px;" onclick="showClickPathModal(event,' +
        k +
        ')">' +
        drawPath(dungeonPaths[currentDungeon].paths[k], false) +
        "</div>";
  ...
  window.drawSymbols = function (list, elementID, key) {
    ...
    for (let k = 0; k < list.length; k++) {
      if (
        ...
        omitForeignSymbols &&
        list[k].basic &&
        !list[k].basic.includes(currentDungeon)
      )
        continue;
      let id = list[k].id;
      let style =
      ...
      if (key === "d")
        style +=
          ...
          "px;";
      s +=
        "<div class='pathsymbol " +
        (key === "d" ? " direction" : "") +
        "' style='" +
        style +
        "' onclick='appendToPath(\"" +
        id +
        "\")'></div>";
    }
  ...
  window.switchScene = function (scene) {
    document.getElementById("overview").style.display =
      scene === "overview" ? "block" : "none";
    document.getElementById("settings").style.display =
      scene === "settings" ? "block" : "none";
    document.getElementById("pathlist").style.display =
      scene === "pathlist" ? "block" : "none";
    document.getElementById("pathedit").style.display =
      scene === "pathedit" ? "block" : "none";
    document.getElementById("owmain").style.display =
      scene === "owmain" ? "block" : "none";
    document.getElementById("owpathedit").style.display =
      scene === "owpathedit" ? "block" : "none";
    document.getElementById("owsearch").style.display =
      scene === "owsearch" ? "block" : "none";
    scrollTo(0, 0);
    if (scene === "overview")
    ...
    if (scene === "pathlist" || scene === "pathedit") {
      document
        .getElementById("sideboss" + currentDungeon)
        .classList.add("activetab");
      document
        .getElementById("sidesummary" + currentDungeon)
        .classList.add("activetab");
    }
  ...
  window.showClickPathModal = function (event, index) {
    ...
    let [drawn, height] = drawPath(
      dungeonPaths[currentDungeon].paths[index],
      true,
      width - 8,
    );
    ...
    document.getElementById("clickPathModalMain").style.top =
      Math.min(event.clientY - 20, window.innerHeight - height - 48) + "px";
  ...
  window.createEntranceNodes = function (showDungeonName, clickAction) {
    ...
    if (lobbyshuffle) {
      switch (currentDungeon) {
        case 2:
          starts +=
            createRoomNode(lobbyEntrances[0], showDungeonName, clickAction) +
            createRoomNode(lobbyHera, showDungeonName, clickAction);
          break;
        case 5:
          for (let k = 1; k < lobbyEntrances.length; k++)
            starts += createRoomNode(
              lobbyEntrances[k],
              showDungeonName,
              clickAction,
            );
      ...
      if (
        (doorshuffle !== "C" || worldState !== "I") &&
        (currentDungeon === 11 || (doorshuffle === "C" && worldState !== "S"))
      )
  ...
  window.loadRoomModalFull = function () {
    ...
    document.getElementById("dungeonimportantmodal").innerHTML =
      createImportantRoomNodeList("more", roomModalClickAction);
  };
  ...
  window.loadRoomModalDefault = function () {
    ...
    document.getElementById("roommodalimportant").classList.remove("selected");
    document.getElementById("dungeonimportantmodal").innerHTML =
      createImportantRoomNodeList("default", roomModalClickAction);
  };
  ...
  window.loadRoomModalNonDefault = function () {
    ...
    document.getElementById("roommodalimportant").classList.remove("selected");
    document.getElementById("dungeonimportantmodal").innerHTML =
      createImportantRoomNodeList("nondefault", roomModalClickAction);
  };
  ...
  window.loadRoomModalImportant = function () {
    ...
    document.getElementById("dungeonimportantmodal").innerHTML =
      createImportantRoomNodeList("fewer", roomModalClickAction);
  };
  ...
  window.showRoomModal = function () {
    ...
    document.getElementById("roommodalswitcher").style.display =
      doorshuffle === "C" ? "flex" : "none";
    document.getElementById("dungeonentrancesmodal").innerHTML =
      createEntranceNodes(doorshuffle === "C", "append");
    loadRoomModalInit();
    document.getElementById("dungeonusedmodal").innerHTML =
      createUsedRoomNodeList("append");
    document.getElementById("roomModalMain").style.top =
      doorshuffle === "C" ? "60px" : "120px";
  ...
  window.showRoomModalPreview = function () {
    ...
    document.getElementById("dungeonentrancesmodal").innerHTML = createRoomNode(
      lobbyEntrances[0],
      true,
      null,
    );
  ...
  window.updateOverworldShuffle = function (showVanillaOWBox) {
    ...
    document.getElementById("customizerpreset").style.display = mixedow
      ? "none"
      : "";
    document.getElementById("customizernotsupported").style.display = mixedow
      ? ""
      : "none";
    if (
      layoutshuffle === "N" &&
      ...
      document.getElementById("overworldoptionsfinalbox").style.display =
        "none";
    else if (showVanillaOWBox) checkAutoAdjustments(false);
    document.getElementById("vanillawelcome").style.display =
      layoutshuffle === "N" && crossedow !== "C" ? "none" : "none";
    updateOverviewElements();
    sendUpdate();
  ...
  window.updateSidebarElements = function () {
    document.getElementById("sidebaredit").style.display =
      document.getElementById("owsidebaredit").checked ? "block" : "none";
    document.getElementById("sidebarnew").style.display =
      (layoutshuffle !== "N" || whirlpoolshuffle) &&
      document.getElementById("owsidebarnew").checked
        ? "block"
        : "none";
    document.getElementById("sidebarsearch").style.display =
      document.getElementById("owsidebarsearch").checked ? "block" : "none";
  };
  ...
  window.updateOverviewElements = function () {
    ...
    document.getElementById("overviewtitle").innerHTML =
      doorshuffle === "N"
        ...
        : noOverworld
          ? "Dungeon Tracker"
          : "Overworld and Dungeon Tracker";
    document.getElementById("overviewoverworld").style.display = noOverworld
      ? "none"
      : "block";
    document.getElementById("overviewownewpath").style.display =
      layoutshuffle === "N" && !whirlpoolshuffle ? "none" : "block";
    document.getElementById("overviewdungeons").style.display =
      doorshuffle === "N" ? "none" : "block";
    document.getElementById("dungeonoptions").style.display =
      doorshuffle === "N" ? "none" : "block";
    document.getElementById("changeworldstateentrance").style.display =
      document.getElementById("globalsync").checked ? "block" : "none";
    document.getElementById("dungeonstats").style.display =
      doorshuffle === "N" ? "none" : "block";
    document.getElementById("sidebaroverworld").style.display = noOverworld
      ? "none"
      : "block";
    document.getElementById("sidebardungeons").style.display =
      doorshuffle === "N" ? "none" : "block";
  ...
  window.checkIfResponded = function () {
    if (
      awaitingResponse &&
      window.opener &&
      (document.getElementById("itemsync").checked ||
        document.getElementById("connectorsync").checked ||
        document.getElementById("globalsync").checked)
    ) {
  ...
  window.updateGlobalSync = function () {
    ...
    if (document.getElementById("globalsync").checked) {
      document.getElementById("itemsync").disabled = document.getElementById(
        "connectorsync",
      ).disabled = true;
      document.getElementById("selectworldstate").disabled =
        document.getElementById("entranceenabled").disabled = true;
      awaitingNextUpdate = true;
      ...
      document.getElementById("selectworldstate").disabled =
        document.getElementById("entranceenabled").disabled = false;
      awaitingNextUpdate = true;
      if (
        !document.getElementById("itemsync").checked &&
        !document.getElementById("connectorsync").checked &&
  ...
  window.updateConnectorSync = function () {
    ...
    if (
      showFullMapMain &&
      document.getElementById("owmain").style.display === "block"
    )
  ...
  window.showGlobalSyncInfo = function () {
    let s =
      "If this is enabled, the main tracker window logic takes Overworld Shuffle and everything tracked in this window into account.\n";
    s +=
    ...
    if (window.opener)
      if (window.opener.closed || awaitingResponse)
        s +=
          "Disconnected from main tracker window, please save your progress, close this tracker window and then open it again from the main tracker window.";
      else s += "Connected to main tracker window, sync possible.";
    else
      s +=
        "Not opened from main tracker window, please close this tracker window and open it from the main tracker window obtained through the Community Tracker launcher.";
    alert(s);
  };
  ...
  window.receiveItemUpdate = function (newItems) {
    ...
    if (document.getElementById("globalsync").checked) {
      ...
      if (newItems.flags && newItems.flags.hasOwnProperty("entrancemode")) {
        ...
        if (
          (newState === "S") != entranceEnabled &&
          (newState === "N" || newState === "S")
        ) {
    ...
    if (
      ...
      for (let k = 0; k < newItems.connectorOne.length; k++) {
        ...
        if (createNew) {
          createConnectors.push([
            newItems.connectorOne[k],
            newItems.connectorTwo[k],
            id,
          ]);
          changed = true;
        }
      ...
      if (deleted) {
        ...
        for (let c of keepConnectors) {
          ...
          createEntranceConnector(
            overworldScreens.get(screenID1).regions.get(regionName1),
            overworldScreens.get(screenID2).regions.get(regionName2),
            c.status,
            c.id,
          );
        }
      ...
      for (let c of createConnectors)
        createEntranceConnector(
          getEntranceRegionFromIndex(c[0]),
          getEntranceRegionFromIndex(c[1]),
          "unknown",
          c[2],
        );
    }
  ...
  window.keyDown = function (event) {
    if (
      ...
      [32, 33, 34, 35, 36, 38, 40].indexOf(event.keyCode) != -1
    )
    ...
    if (window.origin === event.origin) {
      ...
      if (event.data === "MYSTERYRELOAD") {
        if (
          ...
          testConnection(true);
          alert(
            "The main tracker window has been refreshed and disconnected from this window as a consequence of changing certain mode settings.\n\n" +
              reconnectHelpMessage,
          );
        }
      } else if (
        event.data.dungeonPaths &&
        event.data.dungeonPaths.length === 13
      ) {
  ...
  window.loadAllTrack = function (newData) {
    ...
    for (let k = 0; k < 13; k++)
      if (
        !(k in newDungeonPaths) ||
        !Array.isArray(newDungeonPaths[k].paths) ||
        !(
          typeof newDungeonPaths[k].notes === "string" ||
          newDungeonPaths[k].notes instanceof String
        )
      )
        return false;
    ...
    try {
      for (let regionData of newData.customStartRegions)
        customStartRegions.push(
          overworldScreens.get(regionData[0]).regions.get(regionData[1]),
        );
    } catch (error) {
      console.log(error);
    }
    ...
    document.getElementById("owpathsnote").value =
      typeof newOWNotes === "string" || newOWNotes instanceof String
        ? newOWNotes
        : "";
    customFluteSpots = [];
    try {
    ...
    if (document.getElementById("globalsync").checked) {
      document.getElementById("itemsync").disabled =
        document.getElementById("connectorsync").disabled =
        document.getElementById("selectworldstate").disabled =
        document.getElementById("entranceenabled").disabled =
          true;
    } else {
      document.getElementById("itemsync").disabled =
        document.getElementById("connectorsync").disabled =
  ...
  window.loadAllDisplay = function (newData) {
    ...
    if (
      newData.defaultSync &&
      (newData.defaultSync === "everything" ||
        newData.defaultSync === "itemsentrances" ||
        newData.defaultSync === "items" ||
        newData.defaultSync === "entrances" ||
        newData.defaultSync === "nothing")
    )
      document.getElementById("settingsdefaultsync").value = defaultSync =
        newData.defaultSync;
    if (
      newData.drawFluteSpots &&
      (newData.drawFluteSpots === "no" ||
        newData.drawFluteSpots === "fluteshuffle" ||
        newData.drawFluteSpots === "yes")
    )
      document.getElementById("settingsdrawflutespots").value = drawFluteSpots =
        newData.drawFluteSpots;
    if (
      newData.drawSaveQuitSpots &&
      (newData.drawSaveQuitSpots === "no" ||
        newData.drawSaveQuitSpots === "startshuffle" ||
        newData.drawSaveQuitSpots === "yes")
    )
      document.getElementById("settingsdrawsavequitspots").value =
        drawSaveQuitSpots = newData.drawSaveQuitSpots;
    if (
      newData.overworldMainColumns &&
      (newData.overworldMainColumns === "auto" ||
        newData.overworldMainColumns === "1" ||
        newData.overworldMainColumns === "2")
    )
      document.getElementById("settingsmaincolumns").value =
        overworldMainColumns = newData.overworldMainColumns;
    if (
      newData.searchStartDefault &&
      (newData.searchStartDefault === "canstartcommon" ||
        newData.searchStartDefault === "onlystartscreen" ||
        newData.searchStartDefault === "noflutequit")
    )
      document.getElementById("settingssearchstartdefault").value =
        searchStartDefault = newData.searchStartDefault;
    if (
      newData.importantRoomNodes &&
      (newData.importantRoomNodes === "fewer" ||
        newData.importantRoomNodes === "default" ||
        newData.importantRoomNodes === "more")
    )
      document.getElementById("settingsimportantroomnodes").value =
        importantRoomNodes = newData.importantRoomNodes;
    showFullMapMain = newData.showFullMapMain;
    document.getElementById("togglefullowmain").className = showFullMapMain
      ? "cell buttonsquare collapsesection"
      : "cell buttonsquare expandsection";
    document.getElementById("fullowmainpage").style.display = showFullMapMain
      ? "block"
      : "none";
    expandedMainTable = newData.expandedMainTable;
    document.getElementById("toggleowmaintable").className = expandedMainTable
      ? "cell buttonsquare collapsesection"
      : "cell buttonsquare expandsection";
    document.getElementById("owmaintable").style.display = expandedMainTable
      ? "block"
      : "none";
    document.getElementById("owmaintablemini").style.display = expandedMainTable
      ? "none"
      : "block";
    showChecklist = newData.showChecklist;
    document.getElementById("togglechecklist").className = showChecklist
      ? "cell buttonsquare collapsesection"
      : "cell buttonsquare expandsection";
    document.getElementById("checklist").style.display = showChecklist
      ? "block"
      : "none";
    for (let checkboxFlag of checkboxFlagsDisplay)
      if (newData[checkboxFlag] === false || newData[checkboxFlag] === true)
        document.getElementById(checkboxFlag).checked = newData[checkboxFlag];
    invertDirectionColors = document.getElementById(
      "settingsinvertdirectioncolors",
    ).checked;
    roundDirections = document.getElementById(
      "settingsrounddirections",
    ).checked;
    restoreWindowSize = document.getElementById(
      "settingsrestorewindowsize",
    ).checked;
    ...
    preventClickEdgeScreen = document.getElementById(
      "settingspreventclickedgescreen",
    ).checked;
    disableBlur = document.getElementById("settingsdisableblur").checked;
    drawUnshuffledEdges = document.getElementById(
      "settingsdrawunshufflededges",
    ).checked;
    searchMirrorPortalDefault = document.getElementById(
      "settingssearchmirrorportaldefault",
    ).checked;
    omitForeignSymbols = document.getElementById(
      "settingsomitforeignsymbols",
    ).checked;
    ...
    fullZoomMain = newData.fullZoomMain
      ? Math.max(0.5, Math.min(newData.fullZoomMain, 1.5))
      : 0.8;
    fullZoomPopout = newData.fullZoomPopout
      ? Math.max(0.5, Math.min(newData.fullZoomPopout, 1.5))
      : 0.8;
    pathZoom = newData.pathZoom
      ? Math.max(0.5, Math.min(newData.pathZoom, 2))
      : 1;
    fullPathCompact = newData.fullPathCompact;
    try {
      if (
        restoreWindowSize &&
        newData.outerWidth > 0 &&
        newData.outerHeight > 0
      )
        resizeTo(newData.outerWidth, newData.outerHeight);
    } catch (error) {}
    document.getElementById("fullowmainsettingsbutton").style.display =
      showFullMapMain && hideTopBar && mapModeMain !== "single"
        ? "block"
        : "none";
    updateTheme();
  ...
  window.loadManual = function () {
    ...
    if (
      data &&
      confirm("Load saved data from local storage?" + getLoadText(data))
    )
  ...
  window.loadAuto = function () {
    ...
    if (
      data &&
      confirm("Load auto-save data from local storage?" + getLoadText(data))
    )
      if (loadAllTrack(data)) loadOverview();
  ...
  window.loadFile = function () {
    ...
    input.onchange = (event1) => {
      ...
      reader.onload = (event2) => {
        ...
        if (
          data &&
          confirm("Load data from the selected file?" + getLoadText(data))
        )
          if (loadAllTrack(data)) loadOverview();
  ...
  window.saveCustomizerYAML = function () {
    ...
    s += "    ow_mixed: " + mixedow + "\n";
    s +=
      "    ow_fluteshuffle: " + (fluteshuffle ? "balanced" : "vanilla") + "\n";
    ...
    if (mixedow) {
      ...
      for (let [id, screen] of overworldScreens)
        if (!screen.darkWorld && id !== 0x82) {
          ...
          if (screen.mixedState === "swapped") swappedScreens.push(id);
        }
      if (normalScreens.length == 0) s += "    force_no_flip: []\n";
        ...
        for (let id of normalScreens)
          s +=
            ...
            " # " +
            overworldScreens.get(id).randomizerName +
            "\n";
      }
  ...
  window.clearData = function () {
    if (
      confirm(
        "Delete all currently loaded paths and notes for every dungeon and the overworld?",
      )
    ) {
      dungeonPaths = [];
      for (let k = 0; k < 13; k++)
  ...
  window.clearLocalStorage = function () {
    if (
      confirm(
        "Delete everything saved in local storage, including manual save, auto-save and display settings?",
      )
    ) {
      localStorage.clear();
  ...
  window.resetDisplayCheckboxFlags = function () {
    document.getElementById("settingsinvertdirectioncolors").checked =
      invertDirectionColors;
    document.getElementById("settingsrounddirections").checked =
      roundDirections;
    document.getElementById("settingsrestorewindowsize").checked =
      restoreWindowSize;
    ...
    document.getElementById("settingspreventclickedgescreen").checked =
      preventClickEdgeScreen;
    document.getElementById("settingsdisableblur").checked = disableBlur;
    document.getElementById("settingsdrawunshufflededges").checked =
      drawUnshuffledEdges;
    document.getElementById("settingssearchmirrorportaldefault").checked =
      searchMirrorPortalDefault;
    document.getElementById("settingsomitforeignsymbols").checked =
      omitForeignSymbols;
  ...
  window.resetPathZoom = function () {
    if (
      confirm(
        "Reset the zoom level for overworld paths to the default value in search results and saved path lists?",
      )
    ) {
      pathZoom = 1;
      applyOverworldPathZoom();
  ...
  window.welcomeStart = function (modeFinal) {
    ...
    if (
      !entranceEnabled &&
      (worldState === "S" || worldState === "I" || doorshuffle === "N")
    )
  ...
  window.start = function () {
    ...
    if (layoutshuffle !== "N" && layoutshuffle !== "P" && layoutshuffle !== "F")
      layoutshuffle = "N";
    if (
      (query.wild_keys + "").toLowerCase() === "true" ||
      query.world_state === "R" ||
      query.world_state === "r"
    )
    ...
    if (
      ...
      worldState = "O";
    if (
      query.entrance_shuffle &&
      query.entrance_shuffle !== "N" &&
      query.entrance_shuffle !== "n"
    )
    ...
    try {
      if (localStorage.getItem("dungeonData")) {
        document
          .getElementById("loadmanualwelcome")
  ...
  window.loadGeneralSettings = function () {
    ...
    document
      .getElementById("settingstaboverworld")
  ...
  window.loadOverworldSettings = function () {
    ...
    document.getElementById("settingssearchmirrorportaldefault").checked =
      searchMirrorPortalDefault;
    document.getElementById("settingssearchstartdefaulttext").innerHTML =
      "Explanation: " + searchStartDefaultHelp[searchStartDefault];
  };
  ...
  window.loadDoorSettings = function () {
    ...
    document.getElementById("settingsomitforeignsymbols").checked =
      omitForeignSymbols;
    document.getElementById("settingsimportantroomnodestext").innerHTML =
      "Description: " + importantRoomNodesHelp[importantRoomNodes];
  };
  ...
  window.updateOverworldSettings = function () {
    ...
    drawSaveQuitSpots = document.getElementById(
      "settingsdrawsavequitspots",
    ).value;
    hideTopBar = document.getElementById("settingshidetopbar").checked;
    preventClickEdgeScreen = document.getElementById(
      "settingspreventclickedgescreen",
    ...
    searchStartDefault = document.getElementById(
      "settingssearchstartdefault",
    ).value;
    searchMirrorPortalDefault = document.getElementById(
      "settingssearchmirrorportaldefault",
    ).checked;
  ...
  window.updateDoorSettings = function () {
    importantRoomNodes = document.getElementById(
      "settingsimportantroomnodes",
    ).value;
    omitForeignSymbols = document.getElementById(
      "settingsomitforeignsymbols",
    ).checked;
  ...
  window.updateTheme = function () {
    ...
    for (let k of [2, 5, 8, 11])
      directions[k].file = roundDirections
        ? "arrowrightdownround"
        : "arrowrightdown";
    for (let k of [1, 4, 7, 10])
      directions[k].file = roundDirections
        ? "arrowrightupround"
        : "arrowrightup";
    document.getElementById("arrowpreview1").style.backgroundImage =
      roundDirections
        ? "url(./images/dungeons/arrowrightupround.png)"
        : "url(./images/dungeons/arrowrightup.png)";
    document.getElementById("arrowpreview2").style.backgroundImage =
      roundDirections
        ? "url(./images/dungeons/arrowrightdownround.png)"
        : "url(./images/dungeons/arrowrightdown.png)";
  ...
  window.openModeSettings = function () {
    ...
    scrollTo(
      0,
      document.getElementById("modesettings").getBoundingClientRect().top - 36,
    );
  };
  ...
  window.openSyncSettings = function () {
    ...
    scrollTo(
      0,
      document.getElementById("syncsectiontitle").getBoundingClientRect().top -
        36,
    );
  };
  ...
  window.openItems = function () {
    ...
    scrollTo(
      0,
      document.getElementById("overworlditems").getBoundingClientRect().top -
        64,
    );
  };
  ...
  window.openConnectors = function () {
    ...
    scrollTo(
      ...
      document
        .getElementById(
          entranceEnabled ? "owentrancecontainer" : "dungeonconnectorsdefault",
        )
  ...
  window.openCommonStarts = function () {
    ...
    scrollTo(
      0,
      document.getElementById("commonstartinglocations").getBoundingClientRect()
        .top - 64,
    );
  };
  ...
  window.loadOverworld = function () {
    currentDungeon = -1;
    currentOWPath = null;
    document.getElementById("owmaintablenewpath").style.display =
      layoutshuffle === "N" && !whirlpoolshuffle ? "none" : "table-row";
    document.getElementById("owmaintablemininewpath").style.display =
      layoutshuffle === "N" && !whirlpoolshuffle ? "none" : "";
    document.getElementById("pinnedlinkshouse").nextSibling.innerHTML =
      worldState === "I" ? "Bomb Shop" : "Link's House";
    document.getElementById("pinnedsanctuary").nextSibling.innerHTML =
      worldState === "I" ? "Dark Chapel" : "Sanctuary";
    document.getElementById("pinnedoldman").nextSibling.innerHTML =
      worldState === "I" ? "Mountain Cave" : "Mountain Cave";
    document.getElementById("pinnedpyramid").nextSibling.innerHTML =
      worldState === "I" ? "Castle" : "Pyramid";
    document.getElementById("dungeonconnectorsdefault").style.display =
      entranceEnabled ? "none" : "block";
    document.getElementById("dungeonconnectorsdoors").style.display =
      entranceEnabled || doorshuffle === "N" ? "none" : "block";
    document.getElementById("owentrancecontainer").style.display =
      entranceEnabled ? "block" : "none";
    document.getElementById("owfluteshufflerow").style.display = fluteshuffle
      ? "block"
      : "none";
    document.getElementById("owpreviouspathscontainer").style.display =
      layoutshuffle === "N" && !whirlpoolshuffle ? "none" : "block";
    document.getElementById("owbulkactionsrow").style.display =
      layoutshuffle === "N" &&
      !whirlpoolshuffle &&
      (crossedow !== "C" || swappedow)
        ? "none"
        : "";
    updateConnectorList();
    updateStartRegionList();
  ...
  window.loadFullOverworldMain = function () {
    fullOWSelectedScreen =
      fullOWSelectedEdge =
      fullOWConnectorStart =
      fullOWFixedEdge =
        null;
    document.getElementById("fullowmainscreenactions").style.display = "none";
    document.getElementById("fullowmainnewpath").style.display =
      layoutshuffle === "N" && !whirlpoolshuffle ? "none" : "block";
    document.getElementById("fullowmainmixedactions").style.display = mixedow
      ? "block"
      : "none";
    document.getElementById("fullowmainswapscreenwithedges").style.display =
      layoutshuffle !== "N" || (crossedow === "C" && !swappedow)
        ? "block"
        : "none";
    document.getElementById("fullowmainnewconnector").style.display =
      entranceEnabled &&
      !document.getElementById("connectorsync").checked &&
      !document.getElementById("globalsync").checked
        ? "block"
    ...
    document.getElementById("fullowmainconnectoractions").style.display =
      "none";
    document.getElementById("fullowmainpathactions").style.display = fullOWPath
      ? "block"
      : "none";
    document.getElementById("fullowmainrepeatsearch").style.display =
      fullOWPath && fullOWPath.search ? "block" : "none";
  ...
  window.toggleOverworldModeSection = function () {
    ...
    document.getElementById("toggleowmodesection").className =
      showOverworldModeSection
        ? "buttonsquare collapsesection"
        : "buttonsquare expandsection";
    document.getElementById("overworldmodehelp").style.display =
      showOverworldModeSection ? "inline-block" : "none";
    document.getElementById("overworldmodesection").style.display =
      showOverworldModeSection ? "block" : "none";
  };
  ...
  window.updateConnectorList = function () {
    document.getElementById("owconnectorlisttext").innerHTML =
      "Connector caves (" + entranceConnectors.length + ")";
    if (
      document.getElementById("connectorsync").checked ||
      document.getElementById("globalsync").checked
    )
      ...
      document
        .getElementById("owmainaddconnector")
        .classList.remove("disabled");
    let s = "";
    for (let k = 0; k < entranceConnectors.length; k++) {
      s +=
        "<div class='connector'" +
        (entranceConnectors[k].status === "unknown"
          ? " style='border-left: 4px solid yellow; padding-left: 4px; margin: 2px 0;'"
          : "");
      s +=
        "onclick='showClickConnectorModal(event," +
        k +
        ")'><div class='row' style='max-width: 400px;'><div style='display: inline-block;'>";
      ...
      if (entranceConnectors[k].status === "unknown") {
        s +=
          "<div class='row' style='max-width: 400px;'><div style='display: inline-block;'>";
        s +=
          "<div class='cell' style='line-height: 34px; margin-right: 8px;'>Currently possible backwards?</div>";
        s +=
          "<div class='cell buttonbox' onclick='unknownConnectorOneWay(" +
          k +
          "); event.stopPropagation()'>No</div>";
        s +=
          "<div class='cell buttonbox' onclick='unknownConnectorTwoWay(" +
          k +
          "); event.stopPropagation()'>Yes</div></div></div>";
      }
      s += "</div>";
    }
  ...
  window.fullCheckConnectorDetails = function () {
    if (
      entranceEnabled &&
      (useMain || fullOverworldMode === "editedges") &&
      !fullOWSelectedScreen &&
      !fullOWSelectedEdge &&
      !fullOWConnectorStart
    )
      for (let k = entranceConnectors.length - 1; k >= 0; k--)
        if (entranceConnectors[k].status === "unknown") {
          ...
          document.getElementById(
            useMain
              ? "fullowmainunknownconnectortext"
              : "fullowunknownconnectortext",
          ).innerHTML = connectorString(entranceConnectors[k]);
          document.getElementById(
            useMain ? "fullowmainunknownconnector" : "fullowunknownconnector",
          ).style.display = "block";
          return;
        }
    lastUnknownConnectorIndex = -1;
  ...
  window.connectorString = function (connector) {
    let regionStart = overworldScreens
        .get(connector.data[0])
        .regions.get(connector.data[1]),
      regionEnd = overworldScreens
        .get(connector.data[2])
        .regions.get(connector.data[3]);
  ...
  window.clickedConnectorUnknown = function () {
    if (entranceConnectors[clickedConnectorIndex].status === "single") {
      ...
      makeEntranceConnectorUnidirectional(
        entranceConnectors[clickedConnectorIndex],
      );
      entranceConnectors[clickedConnectorIndex].status = "unknown";
      outstandingUpdate = true;
      updateReachableEdges();
  ...
  window.updateStartRegionList = function () {
    ...
    for (let k = 0; k < customStartRegions.length; k++) {
      ...
      s +=
        "<div class='buttonsquare close redborder' style='margin: 2px;' onclick='deleteStartRegion(" +
        k +
        ")'></div></div></div></div>";
    }
      ...
      if (item === "gloves") {
        if (ownItems.mitts) {
          ...
          document
            ...
            .classList.remove("collected", "mitts");
        } else if (ownItems.gloves) {
          ownItems.mitts = true;
          document
            .getElementById("item" + item)
            .classList.add("collected", "mitts");
        } else {
          ownItems.gloves = true;
    ...
    if (main || fullOverworldMode === "editedges") {
      ...
      if (fullOWSelectedEdge) {
        ...
        if (edge === fullOWSelectedEdge) {
          fullOWSelectedEdge = null;
          document.getElementById(mainString + "edgeactions").style.display =
            "none";
          fullCheckConnectorDetails();
        } else {
          if (edge.direction === opposite[fullOWSelectedEdge.direction]) {
            connectSimilarSwappedParallel(
              fullOWSelectedEdge,
              edge,
              decoupledow === "N",
              decoupledow !== "C",
        ...
        if (edgeString === "ZW" ? !whirlpoolshuffle : layoutshuffle === "N") {
          ...
          if (
            similarow &&
            decoupledow !== "C" &&
            (screenID % 0x40 === 0x28 || screenID % 0x40 === 0x29) &&
            (edge.vanilla.screen.id % 0x40 === 0x28 ||
              edge.vanilla.screen.id % 0x40 === 0x29)
          )
            edge =
              edge.direction === "E"
                ? overworldScreens.get(0x68).edges.get("E0")
                : overworldScreens.get(0x69).edges.get("W0");
          let edgeVanilla =
            !edge.parallel ||
            isDarkWorld(edge.screen) === isDarkWorld(edge.vanilla.screen)
              ? edge.vanilla
              : edge.parallel.vanilla;
          if (edge.out) {
            if (edge.out === edgeVanilla && edge.parallel) {
              connectSimilarSwappedParallel(
                edge,
                edgeVanilla.parallel,
                decoupledow === "N",
                decoupledow !== "C",
              );
              outstandingUpdate = true;
          ...
          fullOWSelectedEdge = overworldScreens
            .get(screenID)
            .edges.get(edgeString);
          fullOWSelectedScreen = null;
          document.getElementById(mainString + "screenactions").style.display =
            "none";
          document.getElementById(mainString + "edgeactions").style.display =
            "block";
          document.getElementById(
            mainString + "unknownconnector",
          ).style.display = "none";
  ...
  window.updateClickScreenOptions = function (id) {
    ...
    if (fullOWConnectorStart) {
      if (
        screen.entranceRegions.length !== 0 &&
        fullOWConnectorStart.entranceRegions.length !== 0
      ) {
      ...
      if (screen === fullOWSelectedScreen) {
        ...
        if (screen.entranceRegions.length !== 0)
          document
            .getElementById(mainString + "newconnector")
            .classList.remove("disabled");
        else
          document
            .getElementById(mainString + "newconnector")
        ...
        document.getElementById(mainString + "screenactions").style.display =
          "block";
        document.getElementById(mainString + "screenname").innerHTML =
          screen.name;
        document.getElementById(mainString + "mixedscreenstate").innerHTML =
          "Mixed state: " + screen.mixedState;
        updateClickScreenCommonStarts();
        document.getElementById(mainString + "unknownconnector").style.display =
          "none";
      }
  ...
  window.updateClickScreenCommonStarts = function () {
    if (fullOWSelectedScreen) {
      let mainString = useMain ? "fullowmain" : "fullow";
      let s =
        "<div class='cell textrow' style='line-height: 16px; margin: 4px;'>Add/remove common start:</div>";
      for (let [regionName, region] of fullOWSelectedScreen.regions)
        if (allStartRegions.includes(region))
          s +=
            "<div class='buttonbox small redborder' style='float: left; margin: 2px;' onclick='removeStartRegion(this,\"" +
            regionName +
            "\")'>" +
            regionName +
            "</div>";
          ...
          s +=
            "<div class='buttonbox small' style='float: left; margin: 2px;' onclick='addStartRegion(this,\"" +
            regionName +
            "\")'>" +
            regionName +
            "</div>";
  ...
  window.updateEdgeActions = function (main) {
    let mainString = main ? "fullowmain" : "fullow";
    document.getElementById(mainString + "edgeloop").style.display =
      fullOWSelectedEdge.string === "ZW" ? "block" : "none";
    document.getElementById(mainString + "edgecoupled").style.display =
      decoupledow !== "N" ? "none" : "block";
    document.getElementById(mainString + "edgedecoupled").style.display =
      decoupledow !== "N" ? "block" : "none";
    document.getElementById(mainString + "edgename").innerHTML =
      getNiceEdgeName(fullOWSelectedEdge);
    if (decoupledow === "N") {
      if (fullOWSelectedEdge.out) {
        document.getElementById(mainString + "edgetext").innerHTML =
          "Clicking on another edge will replace the old connection.";
        document.getElementById(mainString + "edgecoupledtext").style.display =
          "none";
        document.getElementById(mainString + "edgecoupledname").innerHTML =
          getNiceEdgeName(fullOWSelectedEdge.out);
        document.getElementById(mainString + "edgecoupledname").style.display =
          "";
        document
          .getElementById(mainString + "edgecoupleddelete")
          .classList.remove("disabled");
      } else {
        document.getElementById(mainString + "edgetext").innerHTML =
          "Click on another edge to set the destination.";
        document.getElementById(mainString + "edgecoupledtext").style.display =
          "";
        document.getElementById(mainString + "edgecoupledname").style.display =
          "none";
      ...
      document
        .getElementById(mainString + "edgecoupledname")
        .setAttribute(
          "onmouseover",
          "hoverEdgeOut(" +
            fullOWSelectedEdge.screen.id +
            ",'" +
            fullOWSelectedEdge.string +
            "')",
        );
      document
        .getElementById(mainString + "edgecoupleddelete")
      ...
      if (fullOWSelectedEdge.out) {
        document.getElementById(
          mainString + "edgedecoupledouttext",
        ).style.display = "none";
        document.getElementById(mainString + "edgedecoupledoutname").innerHTML =
          getNiceEdgeName(fullOWSelectedEdge.out);
        document.getElementById(
          mainString + "edgedecoupledoutname",
        ).style.display = "";
        document
          .getElementById(mainString + "edgedecoupledoutdelete")
          .classList.remove("disabled");
      } else {
        document.getElementById(
          mainString + "edgedecoupledouttext",
      ...
      if (decoupledow === "C" || fullOWSelectedEdge.in.length > 1) {
        document.getElementById(
          mainString + "edgedecoupledinonerow",
        ).style.display = "none";
        document.getElementById(
          mainString + "edgedecoupledinrow",
        ).style.display = "block";
      } else {
        document.getElementById(
          mainString + "edgedecoupledinonerow",
      ...
      if (fullOWSelectedEdge.in.length) {
        document.getElementById(
          mainString + "edgedecoupledinonetext",
        ).style.display = "none";
        document.getElementById(
          mainString + "edgedecoupledinonename",
        ).innerHTML = getNiceEdgeName(fullOWSelectedEdge.in[0]);
        document.getElementById(
          mainString + "edgedecoupledinonename",
        ).style.display = "";
        document
          .getElementById(mainString + "edgedecoupledinonedelete")
          .classList.remove("disabled");
        document.getElementById(mainString + "edgedecoupledintext").innerHTML =
          fullOWSelectedEdge.in.length +
          (fullOWSelectedEdge.in.length === 1 ? " edge:" : " edges:");
        document
          .getElementById(mainString + "edgedecoupledindelete")
          .classList.remove("disabled");
        let s = "";
        for (let k = 0; k < fullOWSelectedEdge.in.length; k++) {
          s +=
            "<div class='cell' onmouseover='hoverEdgeInSingle(" +
            fullOWSelectedEdge.screen.id +
            ',"' +
            fullOWSelectedEdge.string +
            '",' +
            k +
            ")' onmouseout='clearFullOverlay(" +
            main +
            ")'>";
          s +=
            "<div class='cell buttonbox dense' onclick='fullOverworldSelectEdgeIn(" +
            k +
            ")'>" +
            getNiceEdgeName(fullOWSelectedEdge.in[k]) +
            "</div>";
          s +=
            "<div class='cell buttonsquare close redborder dense' onclick='fullOverworldDeleteConnectionSingleIn(" +
            k +
            ")'></div></div>";
        }
        document.getElementById(
          mainString + "edgedecoupledinbuttons",
        ).innerHTML = s;
      } else {
        document.getElementById(
          mainString + "edgedecoupledinonetext",
      ...
      document
        .getElementById(mainString + "edgedecoupledoutname")
        .setAttribute(
          "onmouseover",
          "hoverEdgeOut(" +
            fullOWSelectedEdge.screen.id +
      ...
      document
        .getElementById(mainString + "edgedecoupledinonename")
        .setAttribute(
          "onmouseover",
          "hoverEdgeInAll(" +
            fullOWSelectedEdge.screen.id +
  ...
  window.fullOverworldUnknownState = function () {
    ...
    fullOWSelectedScreen = null;
    document.getElementById(
      useMain ? "fullowmainscreenactions" : "fullowscreenactions",
    ).style.display = "none";
    fullCheckConnectorDetails();
    updateReachableEdges();
    drawFullOverworldPanels();
  ...
  window.fullOverworldSwapScreen = function (button, includeEdges) {
    if (
      mixedow &&
      fullOWSelectedScreen &&
      fullOWSelectedScreen.mixedState !== "unknown"
    ) {
      if (includeEdges) {
          ...
          if (id < 0x40 || id >= 0x80) {
            ...
            for (let edge of screen.edges.values())
              if (
                edge.parallel &&
                (!similarow ||
                  (!gameLeft && (!gameRight || edge.direction !== "W")))
              ) {
                if (
                  (edge.out && !group.includes(edge.out.screen.id)) ||
                  (edge.parallel.out &&
                    !group.includes(edge.parallel.out.screen.id))
                ) {
                  let old = edge.out;
                ...
                for (let e of edge.in)
                  if (!group.includes(e.screen.id)) edgeList1.push(e);
      ...
      setMixedScreen(
        fullOWSelectedScreen,
        fullOWSelectedScreen.mixedState === "normal" ? "swapped" : "normal",
      );
      outstandingUpdate = true;
      buttonFlash(button);
      updateReachableEdges();
      drawFullOverworldPanels();
      document.getElementById(
        useMain ? "fullowmainmixedscreenstate" : "fullowmixedscreenstate",
      ).innerHTML = "Mixed state: " + fullOWSelectedScreen.mixedState;
  ...
  window.fullOverworldNewConnector = function () {
    if (
      fullOWSelectedScreen &&
      fullOWSelectedScreen.entranceRegions.length !== 0
    ) {
      ...
      document.getElementById(
        useMain ? "fullowmainconnectoractions" : "fullowconnectoractions",
      ).style.display = "block";
      drawFullOverworldPanelCurrent();
    }
  };
  ...
  window.addStartRegion = function (button, regionName) {
    ...
    button.setAttribute(
      "onclick",
      "removeStartRegion(this,'" + regionName + "')",
    );
  };
  ...
  window.fullOverworldLoopEdge = function () {
    if (
      ...
      fullOWSelectedEdge = null;
      document.getElementById(
        useMain ? "fullowmainedgeactions" : "fullowedgeactions",
      ).style.display = "none";
      fullCheckConnectorDetails();
      drawFullOverworldPanels();
    }
  ...
  window.fullOverworldDeleteConnectionAll = function () {
    if (
      fullOWSelectedEdge &&
      (fullOWSelectedEdge.out || fullOWSelectedEdge.in.length)
    ) {
  ...
  window.fullOverworldDeletePath = function () {
    document.getElementById("fullowmainpanelpathoverlay").innerHTML =
      document.getElementById("fullowpanelpathoverlay").innerHTML = "";
    document.getElementById("fullowmainpathactions").style.display =
      document.getElementById("fullowpathactions").style.display = "none";
    fullOWPath = null;
    document.getElementById("fullowmainhighlightedpath").innerHTML =
      document.getElementById("fullowhighlightedpath").innerHTML = "";
    drawFullOverworldPanels();
  };
  ...
  window.fullOverworldToggleCompactPath = function () {
    if (fullOWPath) {
      ...
      document.getElementById("fullowmainhighlightedpath").innerHTML =
        document.getElementById("fullowhighlightedpath").innerHTML =
          drawOverworldPath(fullOWPath.path, !fullPathCompact, true, false)[0];
    }
  };
  ...
  window.startOverworldPath = function (id) {
    if (layoutshuffle !== "N" || whirlpoolshuffle) {
      ...
      document.getElementById("owcurrentpathheader").innerHTML =
        "Creating a new overworld path";
  ...
  window.drawDirectionsFromScreen = function (screen) {
    let s =
      "<div style='position: absolute; left: 192px; top: 16px;'>" +
      drawSingleOverworldScreen(screen, "alledges") +
      "</div>";
    for (let edgeString of screen.edges.keys()) {
      let symbol = overworldEdgeToDirection[edgeString];
      s += drawDirectionFromSymbol(
        symbol,
        'appendToOverworldPath("' + edgeString + '")',
      );
    }
    if (screen.portal)
      s += drawDirectionFromSymbol(
        overworldEdgeToDirection["PO"],
        'appendToOverworldPath("PO")',
      );
    if (screen.darkWorld)
      s += drawDirectionFromSymbol(
        overworldEdgeToDirection["MI"],
        'appendToOverworldPath("MI")',
      );
  ...
  window.drawDirectionFromSymbol = function (symbol, onClick) {
    ...
    style +=
      " position: absolute; left: " + symbol.x + "px; top: " + symbol.y + "px;";
    return (
      "<div class='pathsymbol' style='" +
      style +
      "' onclick='" +
      onClick +
      "'></div>"
  ...
  window.appendToOverworldPath = function (edgeString) {
    ...
    if (edgeString === "PO" || edgeString === "MI") {
      ...
      let edge = currentOWScreen.edges.get(edgeString),
        targetEdge = null;
      if (
        (edgeString === "ZW" ? !whirlpoolshuffle : layoutshuffle === "N") &&
        (crossedow !== "C" || !edge.parallel)
      )
        targetEdge = getConnectedEdge(edge, false);
      if (
        !targetEdge &&
        edge.out &&
        document.getElementById("alwaysfollowmarked").checked
      )
        targetEdge = edge.out;
      if (targetEdge) {
  ...
  window.saveConnector = function () {
    let radioStart = document.querySelector(
        "input[name='connectorstartgroup']:checked",
      ),
      radioEnd = document.querySelector(
        "input[name='connectorendgroup']:checked",
      );
    if (radioStart && radioEnd) {
      ...
      createEntranceConnector(
        regionStart,
        regionEnd,
        document.getElementById("connectorbidirectional").checked
          ? "both"
          : "single",
        null,
      );
  ...
  window.drawOverworldPath = function (
    path,
    detailed,
    drawNumbers,
    multiline,
      ...
      switch (k % 3) {
        case 0: //Overworld screen
          if (detailed || k == 0 || k == path.length - 1) {
            ...
            if (drawNumbers)
              s +=
                "<div class='pathnumber' style='left: " +
                (left + 24) +
                "px; top: " +
                (top + 24) +
                "px;'>" +
            ...
            if (k != 0 && !specialEdgeTypes.includes(path[k - 1])) {
              ...
              let newX =
                left + edge.x2 * 128 * scale - dotOffsetX[edge.direction];
              let newY =
                top + edge.y2 * 128 * scale - dotOffsetY[edge.direction];
              s +=
                "<div class='owedgedot to' style='left: " +
                (newX - 4) +
                "px; top: " +
                (newY - 4) +
                "px;'></div>";
            ...
            if (
              k != path.length - 1 &&
              !specialEdgeTypes.includes(path[k + 1])
            ) {
              ...
              lastEdgeY =
                top + edge.y2 * 128 * scale + dotOffsetY[edge.direction];
              s +=
                "<div class='owedgedot' style='left: " +
                (lastEdgeX - 4) +
                "px; top: " +
                (lastEdgeY - 4) +
                "px;'></div>";
            ...
            s +=
              "<div class='pathnumber single' style='left: " +
              (left - 8) +
              "px; top: " +
              (top + 40) +
              "px;' onmouseover='hoverPathNumber(" +
              path[k] +
              ")' onmouseout='clearPathNumber()'>" +
              k / 3 +
              "</div>";
          }
          break;
        case 1: //Edge from
          ...
          let extraString =
            drawNumbers && (path[k] === "ZW" || "012".indexOf(path[k][1]) != -1)
              ? "onmouseover='hoverEdgeOut(" +
                path[k - 1] +
                ',"' +
                path[k] +
                "\")' onmouseout='clearFullOverlay(null)'"
              : null;
          s += createSymbolNode(
            overworldEdgeToDirection[path[k]],
            left,
            top,
            false,
            extraString,
  ...
  window.removeLastOverworld = function () {
    if (currentOWPath.length > 1) {
      ...
      fullOWFixedEdge = null;
      currentOWScreen = overworldScreens.get(
        currentOWPath[currentOWPath.length - 1],
      );
      drawDirectionsFromScreen(currentOWScreen);
      updateCurrentOverworldPath();
    }
  ...
  window.startOverworldSearch = function (id) {
    ...
    document.getElementById("owsearchmirrorportals").checked =
      searchMirrorPortalDefault;
  ...
  window.startOverworldSearchPreset = function (presetName) {
    switch (presetName) {
      case "activateflute":
        loadOverworldSearch(
          createSearchPreset(null, null, 0x18, "Main", "none"),
        );
        break;
      case "smith":
        loadOverworldSearch(
          createSearchPreset(0x69, "Frog", 0x22, "Main", "smithfrog"),
        );
        break;
      case "purplechest":
        loadOverworldSearch(
          createSearchPreset(0x62, "Main", 0x3a, "Main", "purplechest"),
        );
        break;
      case "redbomb":
        loadOverworldSearch(
          createSearchPreset(
            worldState === "I" ? 0x2c : 0x6c,
            "Main",
            0x5b,
            "Main",
            "redbomb",
  ...
  window.createSearchPreset = function (
    startID,
    startRegion,
    targetID,
    targetRegion,
  ...
  window.loadOverworldSearch = function (search) {
    ...
    try {
      searchStartScreen =
        search.startScreen || search.startScreen === 0
          ? overworldScreens.get(search.startScreen)
          : null;
      searchStartRegion =
        searchStartScreen &&
        search.startRegion &&
        searchStartScreen.regions.size !== 1
          ? searchStartScreen.regions.get(search.startRegion)
          : null;
      setSearchStartMode(search.startMode);
      currentOWScreen = overworldScreens.get(search.targetScreen);
      searchTargetRegion =
        search.targetRegion && currentOWScreen.regions.size !== 1
          ? currentOWScreen.regions.get(search.targetRegion)
          : null;
      document.getElementById("owsearchmirrorportals").checked =
        !!search.mirrorPortals;
      document.getElementById("owsearchfollower").value = search.follower
        ? search.follower
        : "none";
      document.getElementById("owsearchignoreitemrules").checked =
        !!search.ignoreItemRules;
      document.getElementById("owsearchkeepmirrorportal").checked =
        !!search.keepMirrorPortal;
      switchScene("owsearch");
  ...
  window.saveOverworldSearch = function () {
    ...
    search.mirrorPortals = document.getElementById(
      "owsearchmirrorportals",
    ).checked;
    search.follower = document.getElementById("owsearchfollower").value;
    search.ignoreItemRules = document.getElementById(
      "owsearchignoreitemrules",
    ).checked;
    search.keepMirrorPortal = document.getElementById(
      "owsearchkeepmirrorportal",
    ).checked;
    return search;
  };
  ...
  window.searchOverworldPath = function (
    resetStartRegions,
    resetTargetRegions,
  ) {
    ...
    searchResultsLength = 0;
    searchResultPaths = [];
    document.getElementById("checklistedges").style.display =
      layoutshuffle !== "N" || whirlpoolshuffle || crossedow === "C"
        ? "table-row"
        : "none";
    document.getElementById("checklistmixed").style.display = mixedow
      ? "table-row"
      : "none";
    document.getElementById("checklistflute").style.display = fluteshuffle
      ? "table-row"
      : "none";
    hideFullOverworldModal();
    ...
    if (document.getElementById("owsearchignoreitemrules").checked)
      giveAllItems(items);
    let startRegions = searchStartScreen
      ? searchStartRegion
        ? [searchStartRegion]
        : Array.from(searchStartScreen.regions.values())
      : allStartRegions.concat(fluteSpotRegions);
    ...
    for (let region of screen.regions.values()) {
      options.keepMirrorPortal = items.keepMirrorPortal =
        document.getElementById("owsearchkeepmirrorportal").checked;
      ...
      for (let start of startRegions) {
        ...
        if (
          document.getElementById("owsearchmirrorportals").checked &&
          items.mirror &&
          isDarkWorld(start.screen) === (worldState === "I") &&
          start.parallel &&
          !start.parallel.mirrorBlock &&
          start.parallel.distance != Infinity
        ) {
      ...
      if (
        document.getElementById("owsearchmirrorportals").checked &&
        items.mirror &&
        isDarkWorld(screen) !== (worldState === "I") &&
        region.parallel &&
        !region.mirrorBlock
      ) {
    ...
    if (searchStartScreen)
      document
        .getElementById("owsearchswapbutton")
        .classList.remove("disabled");
    else
      document.getElementById("owsearchswapbutton").classList.add("disabled");
    document.getElementById("owsearchstartscreenname").innerHTML =
      "Start: " +
      (searchStartScreen
        ? searchStartScreen.name
        : "Common starting locations");
    document.getElementById("owsearchstartbutton").innerHTML = searchStartScreen
      ? "Change"
      : "Select screen";
    document.getElementById("owsearchstartpanel").innerHTML = searchStartScreen
      ? drawSingleOverworldScreen(searchStartScreen, "search")
      : "";
    document.getElementById("owsearchstartpanel").style.display =
      searchStartScreen ? "block" : "none";
    document.getElementById("owsearchstartcommonchoice").style.display =
      searchStartScreen ? "block" : "none";
    if (searchStartScreen && searchStartScreen.regions.size != 1) {
      let s =
        "<div id='owstartanyregion' class='cell buttonbox dense' onclick='setSearchStartRegion(null)'>Any region</div>";
      for (let name of searchStartScreen.regions.keys())
        s +=
          "<div id='owstartregion" +
          name +
          "' class='cell buttonbox dense' onclick='setSearchStartRegion(\"" +
          name +
          "\")'>" +
          name +
          "</div>";
      ...
      document
        .getElementById(
          searchStartRegion
            ? "owstartregion" + searchStartRegion.name
            : "owstartanyregion",
        )
    ...
    document.getElementById("owsearchtargetscreenname").innerHTML =
      "Target: " + screen.name;
    document.getElementById("owsearchtargetpanel").innerHTML =
      drawSingleOverworldScreen(screen, "search");
    if (screen.regions.size != 1) {
      let s =
        "<div id='owanyregion' class='cell buttonbox dense' onclick='setSearchTargetRegion(null)'>Any region</div>";
      for (let name of screen.regions.keys())
        s +=
          "<div id='owregion" +
          name +
          "' class='cell buttonbox dense' onclick='setSearchTargetRegion(\"" +
          name +
          "\")'>" +
          name +
          "</div>";
  ...
  window.buildPathFromSearch = function (start) {
    ...
    for (
      ...
      switch (current.nextEdgeType) {
        case "S":
          path.push(
            current.nextEdge.string,
            getConnectedEdge(current.nextEdge, false).string,
            current.nextRegion.screen.id,
          );
          break;
        case "P":
  ...
  window.setSearchStartRegion = function (regionName) {
    searchStartRegion = regionName
      ? searchStartScreen.regions.get(regionName)
      : null;
    if (regionName) {
      ...
      for (let r of searchStartScreen.regions.values())
        if (r === searchStartRegion)
          document
            .getElementById("owstartregion" + r.name)
            .classList.add("selected");
        else
          document
            .getElementById("owstartregion" + r.name)
  ...
  window.setSearchTargetRegion = function (regionName) {
    searchTargetRegion = regionName
      ? currentOWScreen.regions.get(regionName)
      : null;
    updateSearchResults();
  };
  ...
  window.updateSearchResults = function () {
    ...
    if (region) {
      ...
      if (currentOWScreen.regions.size != 1) {
        ...
        for (let r of searchResults.keys())
          if (r === region)
            document
              .getElementById("owregion" + r.name)
              .classList.add("selected");
          else
            document
              .getElementById("owregion" + r.name)
      ...
      for (let list of searchResults.values())
        for (let path of list) paths.push(path);
      if (currentOWScreen.regions.size != 1) {
    ...
    searchResultPaths = paths.map((path) => ({
      path: path[0],
      search: currentSearch,
    }));
    document.getElementById("owsearchresultstext").innerHTML =
      "Search results (" + searchResultsLength + ")";
    document.getElementById("owsearchresults").innerHTML = drawPathList(
      searchResultPaths,
      document.getElementById("compactsearchresults").checked,
      "search",
    );
    document.getElementById("owsearchresults").style.height =
      (searchResultsLength * 66 - 2) * pathZoom + "px";
  };
  ...
  window.searchCanStartCommon = function () {
    ...
    document
      .getElementById("owsearchonlystartscreen")
      .classList.remove("selected");
  ...
  window.searchOnlyStartScreen = function () {
    ...
    document
      .getElementById("owsearchcanstartcommon")
      .classList.remove("selected");
    document
      .getElementById("owsearchonlystartscreen")
      .classList.add("selected");
  ...
  window.drawPathList = function (paths, compact, listName) {
    ...
    for (let k = 0; k < paths.length; k++)
      s +=
        '<div class="row path" style="margin: 2px 0px;" onclick="showClickOverworldPathModal(event,' +
        k +
        ",'" +
        listName +
        "')\">" +
        drawOverworldPath(paths[k].path, !compact, false, false)[0] +
        "</div>";
    return s;
  };
  ...
  window.updateMainPathLists = function () {
    document.getElementById("owpinnedtext").innerHTML =
      "Pinned paths (" + pinnedPaths.length + ")";
    document.getElementById("owpinnedpaths").innerHTML = drawPathList(
      pinnedPaths,
      document.getElementById("compactpinned").checked,
      "pinned",
    );
    document.getElementById("owpinnedpaths").style.height =
      (pinnedPaths.length * 66 - 2) * pathZoom + "px";
    document.getElementById("owprevioustext").innerHTML =
      "Saved explored paths (" + previousPaths.length + ")";
    document.getElementById("owpreviouspaths").innerHTML = drawPathList(
      previousPaths,
      document.getElementById("compactprevious").checked,
      "previous",
    );
    document.getElementById("owpreviouspaths").style.height =
      (previousPaths.length * 66 - 2) * pathZoom + "px";
  };

  window.equalPaths = function (path1, path2) {
    if (path1.length != path2.length) return false;
    for (let k = 0; k < path1.length; k++)
      if (path1[k] != path2[k]) return false;
    return true;
  };
  ...
  window.drawFullOverworldPanel = function (main) {
    ...
    document.getElementById(mainString + "panelpathoverlay").style.display =
      "block";
    let visitedScreenEdges = reachableEdges;
    let fixedScreen = fullOWConnectorStart
      ? fullOWConnectorStart
      : fullOWSelectedScreen;
    let fixedEdge =
      !main && mode === "continuepath" ? fullOWFixedEdge : fullOWSelectedEdge;
    let s = "";
    for (let [id, screen] of overworldScreens) {
      let darkWorld = isDarkWorld(screen);
      if (
        (mapMode === "single" &&
          darkWorld != drawDarkWorld &&
          (!mixedow || !screen.special || screen.mixedState !== "unknown")) ||
        (screen.special && mode === "editflutespots")
      )
        continue;
      let unknownScreen =
        mixedow && screen.mixedState === "unknown" && mode !== "editflutespots";
      let scale = 0.5,
        classes = "owscreen full",
        onClick = unknownScreen
          ? ""
          : "clickScreenFull(event," + id + "," + main + ")",
        valid = true;
      ...
      if (screen.special)
        s +=
          ...
          "px; top: " +
          y +
          "px; z-index: 1'><div class='specialshadow'><img class='roomnodeimg' src='./images/overlay/" +
          screen.file +
          ".png' style='transform: scale(" +
          scale +
          ");'></div>";
      ...
      if (unknownScreen) {
        ...
        s +=
          "<span class='fullmixed normal' " +
          (top
            ? "style='left: " + (left + 4) + "px; top: " + (top + 4) + "px;' "
            : "") +
          "onclick='clickNormalScreen(" +
          id +
          ")'>Normal</span>";
        s +=
          "<span class='fullmixed swapped' " +
          (top
            ? "style='left: " + (left + 4) + "px; top: " + (top + 34) + "px;' "
            : "") +
          "onclick='clickSwappedScreen(" +
          id +
          ")'>Flipped</span>";
        s +=
          "</span><span class='questionmark" +
          (checkableScreens.has(id & 0xbf)
            ? " greentext"
            : maybeCheckableScreens.has(id & 0xbf)
              ? " yellowtext"
              : "") +
          "'" +
        ...
        s +=
          "<label class='ttlabel'" +
          (x + y ? "" : " style='line-height: 144px;'") +
          ">" +
          screen.name +
          "</label>";
      s += "</span></div>";
      if (
        drawSaveQuitSpots === "on" ||
        (drawSaveQuitSpots === "startshuffle" &&
          (entranceEnabled ||
            (doorshuffle !== "N" && worldState !== "S" && worldState !== "I")))
      )
        for (let region of screen.regions.values())
          if (allStartRegions.includes(region)) {
            s +=
              "<div class='savequitsymbol' style='left: " +
              (x + getBigScreenSubareaX(screen) + 16) +
              "px; top: " +
              (y + getBigScreenSubareaY(screen) + 16) +
              "px; opacity: " +
              (mode === "editflutespots" ? 0.25 : 0.5) +
              "'></div>";
            break;
          }
      if (
        (mode === "editflutespots" ||
          ((drawFluteSpots === "on" ||
            (drawFluteSpots === "fluteshuffle" && fluteshuffle)) &&
            ownItems.flute &&
            document.getElementById("activeflutebox").checked)) &&
        darkWorld === (worldState === "I") &&
        (fluteshuffle ? customFluteSpots : vanillaFluteSpots).includes(
          id % 0x40,
        )
      )
        s +=
          "<div class='flutesymbol' style='left: " +
          (x + getBigScreenSubareaX(screen) + 16) +
          "px; top: " +
          (y + getBigScreenSubareaY(screen) + 16) +
          "px; opacity: " +
          (mode === "editflutespots" ? 1 : 0.75) +
          "'></div>";
      if (!fullOWConnectorStart && !unknownScreen && mode !== "editflutespots")
        for (let edge of screen.edges.values()) {
          let isShuffled = isShuffledEdge(edge);
          if (
            (isShuffled || drawUnshuffledEdges) &&
            (!fixedEdge ||
              edge === fixedEdge ||
              edgesCompatible(edge, fixedEdge))
          ) {
            ...
            if (isShuffled) {
              ...
              if (!e) {
                ...
                if (
                  mixedow &&
                  edge.vanilla.screen.mixedState === "unknown" &&
                  !edge.parallel
                )
                  questionMark = true;
              }
            ...
            s +=
              ...
              classes +
              "' style='left: " +
              (x + edge.x * 128 * scale - 8) +
              "px; top: " +
              (y + edge.y * 128 * scale - 8) +
              "px;" +
              (screen.special ? " z-index: 1;" : "") +
              "'";
            if (mode === "editedges" && isShuffled)
              s +=
                " onclick='clickEdgeFull(" +
                id +
                ',"' +
                edge.string +
                '",' +
                main +
                ")' onmouseover='hoverEdge(" +
                id +
                ',"' +
                edge.string +
                "\")' onmouseout='clearFullOverlay(" +
            ...
            if (
              ...
              s +=
                ...
                (screen.special ? " z-index: 1;" : "") +
                "'>" +
                (decoupledow === "C" || edge.in.length > 1
                  ? edge.in.length
                  : "") +
                "</div>";
            }
      ...
      if (
        (id === 0x18 || (id === 0x58 && mapMode === "single")) &&
        mode === "editedges" &&
        !fullOWConnectorStart &&
        ownItems.flute &&
        !document.getElementById("activeflutebox").checked
        ...
        s +=
          ...
          (visitedScreenEdges.has(overworldScreens.get(0x18).edges.get("N0")) &&
          (ownItems.moonpearl ||
            (id === 0x18) === (darkWorld === (worldState === "I")))
            ? " green"
            : "") +
          "' style='left: " +
          (x + 16) +
      ...
      if (
        ...
        s +=
          ...
          "px; top: " +
          (y + 26) +
          "px;' onclick='clickSetFluteSpots()'>Set flute spots</span>";
      if (
        (id === 0x03 || (id === 0x43 && drawDarkWorld)) &&
        mode === "editedges" &&
        !fullOWConnectorStart &&
        document.getElementById("overworldoptionsfinalbox").style.display ===
          "block"
        ...
        s +=
          "<span class='activateflute green' style='left: " +
          (x + 16) +
          "px; top: " +
          (y + 94) +
          "px;' onclick='showAutoAdjustModal()'>Auto fixes</span>";
      if (id % 0x40 === 0x30 && mixedow && !darkWorld)
        s +=
          "<div class='lightworldsymbol' style='left: " +
          (x + 1) +
          "px; top: " +
          (y + 47) +
          "px;'></div>";
      if (id % 0x40 === 0x30 && mixedow && darkWorld)
        s +=
          "<div class='darkworldsymbol' style='left: " +
          (x + 1) +
          "px; top: " +
          (y + 47) +
          "px;'></div>";
      if (indexInPath !== -1)
        s +=
          "<div class='pathnumber' style='left: " +
          (x + getBigScreenSubareaX(screen) + 24) +
          "px; top: " +
          (y + getBigScreenSubareaY(screen) + 24) +
          "px;" +
          (screen.special ? " z-index: 1;" : "") +
          "'>" +
          indexInPath / 3 +
    ...
    if (mode === "editedges" && fullOWPath) {
      ...
      for (let k = 0; k < fullOWPath.path.length; k += 3) {
        if (k != 0 && !specialEdgeTypes.includes(fullOWPath.path[k - 1])) {
          ...
          let x = screen1.special
            ? screen1.x
            : (screen1.id % 0x08) * 128 * scale;
          let y = screen1.special
            ? screen1.y
            : (screen1.id % 0x40 >> 3) * 128 * scale;
          if (mapMode === "horizontal" && isDarkWorld(screen1) != drawDarkWorld)
            x += 1024 * scale;
          if (mapMode === "vertical" && isDarkWorld(screen1) != drawDarkWorld)
            y += 1024 * scale;
          s += drawFullEdgeConnection(
            edge1,
            edge2,
            x,
            y,
            false,
            mapMode != "single" || isDarkWorld(screen1) === isDarkWorld(screen2)
              ? 1
              : 0.5,
          );
          s +=
            "<div class='owedgedot' style='left: " +
            (x + edge1.x * 128 * scale - 4 + dotOffsetX[edge1.direction]) +
            "px; top: " +
            (y + edge1.y * 128 * scale - 4 + dotOffsetY[edge1.direction]) +
            "px;'></div>";
          x = screen2.special ? screen2.x : (screen2.id % 0x08) * 128 * scale;
          y = screen2.special
            ? screen2.y
            : (screen2.id % 0x40 >> 3) * 128 * scale;
          ...
          s +=
            "<div class='owedgedot to' style='left: " +
            (x + edge2.x * 128 * scale - 4 - dotOffsetX[edge2.direction]) +
            "px; top: " +
            (y + edge2.y * 128 * scale - 4 - dotOffsetY[edge2.direction]) +
            "px;'></div>";
        }
      }
    ...
    if (!main && mode === "editflutespots")
      document.getElementById(mainString + "flutespotstext").innerHTML =
        customFluteSpots.length + "/8 flute spots currently set";
  };

  window.getBigScreenSubareaX = function (screen) {
    return screen.big && screen.id % 0x40 !== 0x05
      ? screen.id % 0x40 === 0x1e || screen.id % 0x40 === 0x30
        ? 64
        : 32
      : 0;
  };
  ...
  window.getBigScreenSubareaY = function (screen) {
    return screen.big
      ? screen.id % 0x40 === 0x00 || screen.id % 0x40 === 0x18
        ? 64
        : 32
      : 0;
  };
  ...
  window.drawSingleOverworldScreen = function (screen, mode) {
    ...
    if (mode !== "connector" && mode !== "search")
      for (let [edgeString, edge] of screen.edges)
        if (
          mode === "alledges" ||
          (edge.direction === extraOWDirection && (terrainow || !edge.water))
        ) {
          ...
          let onClick =
            mode === "alledges"
              ? 'appendToOverworldPath("' + edgeString + '")'
              : "selectTargetEdge(" + edge.symbol + ")";
          s +=
            "<div class='" +
            classes +
            "' style='left: " +
            (edge.x2 * 128 * scale - 12) +
            "px; top: " +
            (edge.y2 * 128 * scale - 12) +
            "px;' onclick='" +
            onClick +
            "'></div>";
        }
  ...
  window.drawFullEdgeConnection = function (
    edge1,
    edge2,
    x1,
    y1,
    ...
    let lastEdgeX =
      x1 +
      edge1.x * 128 * scale +
      dotOffsetX[edge1.direction] * (invert ? -1 : 1);
    let lastEdgeY =
      y1 +
      edge1.y * 128 * scale +
      dotOffsetY[edge1.direction] * (invert ? -1 : 1);
    let newX =
      (edge2.screen.special
        ? edge2.screen.x
        : (edge2.screen.id % 0x08) * 128 * scale) +
      edge2.x * 128 * scale -
      dotOffsetX[edge2.direction] * (invert ? -1 : 1);
    let newY =
      (edge2.screen.special
        ? edge2.screen.y
        : (edge2.screen.id % 0x40 >> 3) * 128 * scale) +
      edge2.y * 128 * scale -
      dotOffsetY[edge2.direction] * (invert ? -1 : 1);
    if (
      (main ? mapModeMain === "horizontal" : mapModePopout === "horizontal") &&
      isDarkWorld(edge2.screen) !=
        (main ? drawDarkWorldMain : drawDarkWorldPopout)
    )
      newX += 1024 * scale;
    if (
      (main ? mapModeMain === "vertical" : mapModePopout === "vertical") &&
      isDarkWorld(edge2.screen) !=
        (main ? drawDarkWorldMain : drawDarkWorldPopout)
    )
      newY += 1024 * scale;
  ...
  window.hoverScreen = function (id, main) {
    if (main || fullOverworldMode !== "editflutespots") {
      ...
      if (
        ...
        isDarkWorld(screen) != (main ? drawDarkWorldMain : drawDarkWorldPopout)
      )
        x += 1024 * scale;
      if (
        (main ? mapModeMain === "vertical" : mapModePopout === "vertical") &&
        ...
        if (isShuffledEdge(edge)) {
          if (edge.out) {
            s += drawFullEdgeConnection(
              ...
              y,
              false,
              (main ? mapModeMain !== "single" : mapModePopout !== "single") ||
                isDarkWorld(screen) === isDarkWorld(edge.out.screen)
                ? 1
                : 0.5,
              main,
            );
            let element = document.getElementById(
              "full" + (main ? "main" : "popout") + edge.out.screen.id,
            );
          ...
          for (let e of edge.in) {
            if (decoupledow !== "N" || edge.in.length > 1 || e !== edge.out)
              s += drawFullEdgeConnection(
                ...
                (main
                  ? mapModeMain !== "single"
                  : mapModePopout !== "single") ||
                  isDarkWorld(screen) === isDarkWorld(e.screen)
                  ? 1
                  : 0.5,
                main,
                true,
            ...
            let element = document.getElementById(
              "full" + (main ? "main" : "popout") + e.screen.id,
            );
            if (element) element.classList.add("connectedin");
          }
        }
      document.getElementById(
        main ? "fullowmainpaneloverlay" : "fullowpaneloverlay",
      ).innerHTML = s;
      document.getElementById(
        main ? "fullowmainpanelpathoverlay" : "fullowpanelpathoverlay",
      ).style.display = "none";
    }
  };

  window.drawEdgeFullOverlay = function (
    screen,
    edge,
    drawOut,
    drawIn,
    ...
    if (drawIn)
      for (let e of edge.in) {
        if (
          !inMode ||
          (inMode === "auto"
            ? decoupledow !== "N" || edge.in.length > 1 || e !== edge.out
            : e === inMode)
        )
          s += drawFullEdgeConnection(
            edge,
  ...
  window.clearFullOverlay = function (main) {
    ...
    for (let element of document.getElementById(
      main ? "fullowmainpanelmain" : "fullowpanelmain",
    ).childNodes)
  ...
  window.clearPathNumber = function () {
    let element = document.getElementById(
      "full" + (useMain ? "main" : "popout") + hoveredPathNumber,
    );
  ...
  window.showFullOverworldModal = function (mode) {
    ...
    document.getElementById("fullownewpath").style.display =
      layoutshuffle === "N" && !whirlpoolshuffle ? "none" : "block";
    document.getElementById("fullowmixedactions").style.display = mixedow
      ? "block"
      : "none";
    document.getElementById("fullowswapscreenwithedges").style.display =
      layoutshuffle !== "N" || (crossedow === "C" && !swappedow)
        ? "block"
        : "none";
    document.getElementById("fullownewconnector").style.display =
      entranceEnabled &&
      !document.getElementById("connectorsync").checked &&
      !document.getElementById("globalsync").checked
        ? "block"
    ...
    document.getElementById("fullowsearchpresets").style.display =
      mode === "searchpath" ? "block" : "none";
    document.getElementById("fullowcommonstarts").style.display =
      mode === "searchpathstart" ? "block" : "none";
    document.getElementById("fullowflutespots").style.display =
      mode === "editflutespots" ? "block" : "none";
    document.getElementById("fullowpathactions").style.display =
      mode === "editedges" && fullOWPath ? "block" : "none";
    document.getElementById("fullowrepeatsearch").style.display =
      fullOWPath && fullOWPath.search ? "block" : "none";
    fullCheckConnectorDetails();
  ...
  window.showOverworldConnectorModal = function () {
    document.getElementById("bigowscreenstartpanel").innerHTML =
      drawSingleOverworldScreen(fullOWConnectorStart, "connector");
    document.getElementById("bigowscreenendpanel").innerHTML =
      drawSingleOverworldScreen(fullOWConnectorEnd, "connector");
    document.getElementById("connectorstartscreenname").innerHTML =
      fullOWConnectorStart.name;
    document.getElementById("connectorendscreenname").innerHTML =
      fullOWConnectorEnd.name;
    let s = "";
    for (let region of fullOWConnectorStart.entranceRegions) {
      let id = "cstart" + region.name.replace(/\s+/g, "");
      s +=
        "<div><label><input type='radio' id='" +
        id +
        "' name='connectorstartgroup' value='" +
        region.name +
        "' onclick='updateConnectorButton()'>" +
        region.name +
        " (" +
        region.numberEntrances +
        ")</label></div>";
    }
    ...
    if (fullOWConnectorStart.entranceRegions.length === 1)
      document.getElementById(
        "cstart" +
          fullOWConnectorStart.entranceRegions[0].name.replace(/\s+/g, ""),
      ).checked = true;
    s = "";
    ...
    if (fullOWConnectorEnd.entranceRegions.length === 1)
      document.getElementById(
        "cend" + fullOWConnectorEnd.entranceRegions[0].name.replace(/\s+/g, ""),
      ).checked = true;
  ...
  window.showOverworldExtraModal = function (options) {
    document.getElementById("bigowscreenpanel").innerHTML =
      drawSingleOverworldScreen(extraOWScreen, "extra");
    for (let k = 0; k < 3; k++)
      if (options[k])
  ...
  window.showClickOverworldPathModal = function (event, pathIndex, listName) {
    ...
    let [drawn, , height] = drawOverworldPath(
      clickedOverworldPath.path,
      true,
      false,
      true,
      (width - 8) / pathZoom,
    ...
    height *= pathZoom;
    document.getElementById("currentclickedowpath").style.height =
      height + "px";
    document.getElementById("currentclickedowpath").innerHTML = drawn;
    document.getElementById("saveowpath").style.display =
      listName === "pinned" ? "none" : "block";
    document.getElementById("repeatsearch").style.display =
      listName === "search" || !clickedOverworldPath.search ? "none" : "block";
    document.getElementById("deleteowpath").style.display =
      listName === "search" ? "none" : "block";
    document.getElementById("clickOWPathModalMain").style.left = "28px";
    document.getElementById("clickOWPathModalMain").style.top =
      Math.min(event.clientY - 20, window.innerHeight - height - 48) + "px";
  ...
  window.showClickConnectorModal = function (event, index) {
    ...
    if (regionStart.screen.entranceRegions.length != 1)
      s += " - " + regionStart.name;
    document.getElementById("currentclickedconnectorstart").innerHTML =
      "Start: " + s;
    s = regionEnd.screen.name;
    if (regionEnd.screen.entranceRegions.length != 1)
      s += " - " + regionEnd.name;
    document.getElementById("currentclickedconnectorend").innerHTML =
      "End: " + s;
    document.getElementById("currentclickedconnectordirections").innerHTML =
      "Direction(s): " +
      (connector.status === "both"
        ? "Both"
        : connector.status === "unknown"
          ? "Unknown (treated as one)"
          : "One");
    if (connector.status === "single")
      document.getElementById("clickedconnectorone").classList.add("disabled");
    else
      document
        .getElementById("clickedconnectorone")
        .classList.remove("disabled");
    if (connector.status === "both")
      document.getElementById("clickedconnectortwo").classList.add("disabled");
    else
      document
        .getElementById("clickedconnectortwo")
        .classList.remove("disabled");
    if (connector.status === "unknown")
      document
        .getElementById("clickedconnectorunknown")
        .classList.add("disabled");
    else
      document
        .getElementById("clickedconnectorunknown")
        .classList.remove("disabled");
    document.getElementById("clickedconnectordelete").style.display =
      document.getElementById("connectorsync").checked ||
      document.getElementById("globalsync").checked
        ? "none"
        : "block";
    document.getElementById("clickconnectorModalMain").style.left =
      (visibleSidebar ? "64" : "4") + "px";
    document.getElementById("clickconnectorModalMain").style.top =
      Math.min(event.clientY - 20, window.innerHeight - 144) + "px";
  ...
  window.showEditMapModeModal = function (main) {
    ...
    document.getElementById("mapmodeauto").checked = main
      ? mapModeAutoMain
      : mapModeAutoPopout;
    document.getElementById(
      "mapmode" + (main ? mapModeMain : mapModePopout),
    ).checked = true;
  ...
  window.showAutoAdjustModal = function () {
    ...
    if (info.invalidEdgesCount) {
      document.getElementById("autoadjustinvalidedgeout").innerHTML =
        getNiceEdgeName(info.invalidEdges[0]);
      document
        .getElementById("autoadjustinvalidedgeout")
        .setAttribute(
          "onclick",
          "viewEdgeOnFullMap(" +
            info.invalidEdges[0].screen.id +
            ",'" +
            info.invalidEdges[0].string +
            "')",
        );
      document.getElementById("autoadjustinvalidedgein").innerHTML =
        getNiceEdgeName(info.invalidEdges[0].out);
      document
        .getElementById("autoadjustinvalidedgein")
        .setAttribute(
          "onclick",
          "viewEdgeOnFullMap(" +
            info.invalidEdges[0].out.screen.id +
            ",'" +
            info.invalidEdges[0].out.string +
            "')",
        );
    }
    if (info.invalidIndegreeCount) {
      document.getElementById("autoadjustinvalidindegreein").innerHTML =
        getNiceEdgeName(info.invalidIndegree[0]);
      document
        .getElementById("autoadjustinvalidindegreein")
        .setAttribute(
          "onclick",
          "viewEdgeOnFullMap(" +
            info.invalidIndegree[0].screen.id +
            ",'" +
            info.invalidIndegree[0].string +
            "')",
        );
    }
    document.getElementById("autoadjustinvalidscreen").style.display =
      info.invalidScreensCount ? "block" : "none";
    document.getElementById("autoadjustinconsistentgroup").style.display =
      info.inconsistentGroupsCount ? "block" : "none";
    document.getElementById("autoadjustmissingscreen").style.display =
      info.missingScreensCount ? "block" : "none";
    document.getElementById("autoadjustinvalidedge").style.display =
      info.invalidEdgesCount ? "block" : "none";
    document.getElementById("autoadjustinvalidindegree").style.display =
      info.invalidIndegreeCount ? "block" : "none";
    if (
      !!info.inconsistentSimilarCount +
        !!info.inconsistentSwappedCount +
        !!info.inconsistentParallelCount +
        !!info.inconsistentCoupledCount >
      1
    ) {
      document.getElementById("autoadjustinconsistentedge").style.display =
        "block";
      document.getElementById("autoadjustinconsistentsimilar").style.display =
        "none";
      document.getElementById("autoadjustinconsistentswapped").style.display =
        "none";
      document.getElementById("autoadjustinconsistentparallel").style.display =
        "none";
      document.getElementById("autoadjustinconsistentcoupled").style.display =
        "none";
    } else {
      document.getElementById("autoadjustinconsistentedge").style.display =
        "none";
      document.getElementById("autoadjustinconsistentsimilar").style.display =
        info.inconsistentSimilarCount ? "block" : "none";
      document.getElementById("autoadjustinconsistentswapped").style.display =
        info.inconsistentSwappedCount ? "block" : "none";
      document.getElementById("autoadjustinconsistentparallel").style.display =
        info.inconsistentParallelCount ? "block" : "none";
      document.getElementById("autoadjustinconsistentcoupled").style.display =
        info.inconsistentCoupledCount ? "block" : "none";
    }
    document.getElementById("autoadjustmissingfixed").style.display =
      info.missingFixedCount ? "block" : "none";
    if (
      !!info.missingSimilarCount +
        !!info.missingSwappedCount +
        !!info.missingParallelCount +
        !!info.missingCoupledCount >
      1
    ) {
      document.getElementById("autoadjustmissingedge").style.display = "block";
      document.getElementById("autoadjustmissingsimilar").style.display =
        "none";
      document.getElementById("autoadjustmissingswapped").style.display =
        "none";
      document.getElementById("autoadjustmissingparallel").style.display =
        "none";
      document.getElementById("autoadjustmissingcoupled").style.display =
        "none";
    } else {
      document.getElementById("autoadjustmissingedge").style.display = "none";
      document.getElementById("autoadjustmissingsimilar").style.display =
        info.missingSimilarCount ? "block" : "none";
      document.getElementById("autoadjustmissingswapped").style.display =
        info.missingSwappedCount ? "block" : "none";
      document.getElementById("autoadjustmissingparallel").style.display =
        info.missingParallelCount ? "block" : "none";
      document.getElementById("autoadjustmissingcoupled").style.display =
        info.missingCoupledCount ? "block" : "none";
    }
    document.getElementById("autoadjustnone").style.display = info.hasChanges
      ? "none"
      : "block";
  ...
  window.updateZoomEditMapModeModal = function () {
    ...
    document.getElementById("mapmodezoomvalue").innerHTML =
      "Zoom: " + (fullZoom * 100).toFixed(1) + "%";
    document.getElementById("mapmodezoomauto").checked = editMapModeMain
      ? fullZoomAutoMain
      : fullZoomAutoPopout;
    if (fullZoom <= 0.5)
  ...
  window.zoomAutoFullOverworldMapMode = function (checkbox) {
    editMapModeMain
      ? zoomAutoFullOverworldMain(checkbox)
      : zoomAutoFullOverworldPopout(checkbox);
    updateZoomEditMapModeModal();
    document.getElementById(
      editMapModeMain ? "mainzoomautoow" : "zoomautoow",
  ...
  window.updateConnectorButton = function () {
    if (
      document.querySelector("input[name='connectorstartgroup']:checked") &&
      document.querySelector("input[name='connectorendgroup']:checked")
    )
  ...
  window.pickMapModeMain = function () {
    ...
    let singleZoom =
      Math.max(256, Math.min(Math.min(maxWidth, maxHeight), 768)) / 512;
    let oldMapMode = mapModeMain;
    if (
      Math.max(
        0.5,
        Math.min(maxWidth / 1024, (maxHeight + (hideTopBar ? 34 : 0)) / 512),
      ) >= Math.min((singleZoom * 7) / 8, 0.75)
    )
      mapModeMain = "horizontal";
    else if (
      Math.max(
        0.5,
        Math.min(maxWidth / 512, (maxHeight + (hideTopBar ? 34 : 0)) / 1024),
      ) >= Math.min((singleZoom * 7) / 8, 0.75)
    )
  ...
  window.updateTopBars = function () {
    document.getElementById("fullowmaintitlebar").style.marginBottom =
      showFullMapMain && hideTopBar && mapModeMain !== "single" ? "4px" : "0";
    document.getElementById("fullowmainsettingsbutton").style.display =
      showFullMapMain && hideTopBar && mapModeMain !== "single"
        ? "block"
        : "none";
    document.getElementById("fullowmainsettingsbar").style.display =
      hideTopBar && mapModeMain !== "single" ? "none" : "block";
    document.getElementById("fullowtitlebar").style.marginBottom =
      hideTopBar && mapModePopout !== "single" ? "4px" : "0";
    document.getElementById("fullowsettingsbutton").style.display =
      hideTopBar && mapModePopout !== "single" ? "block" : "none";
    document.getElementById("fullowsettingsbar").style.display =
      hideTopBar && mapModePopout !== "single" ? "none" : "block";
  };
  ...
  window.updateOverworldMainColumns = function () {
    if (
      overworldMainColumns === "2" ||
      (overworldMainColumns === "auto" &&
        document.body.clientWidth - (visibleSidebar ? 72 : 0) >= 850)
    )
      document
        .getElementById("owmaincolumncontainer")
        .classList.add("columns2");
    else
      document
        .getElementById("owmaincolumncontainer")
        .classList.remove("columns2");
  };
  ...
  window.calculateFullZoomMain = function () {
    let maxWidth = document.body.clientWidth - (visibleSidebar ? 72 : 0),
      maxHeight =
        window.innerHeight -
        252 +
        (hideTopBar && mapModeMain !== "single" ? 34 : 0);
  ...
  window.calculateFullZoomPopout = function () {
    let maxWidth = document.body.clientWidth - 14,
      maxHeight =
        window.innerHeight -
        212 +
        (hideTopBar && mapModePopout !== "single" ? 34 : 0);
  ...
  window.applyFullOverworldZoomMain = function () {
    document.getElementById("fullowmainpanel").style.width =
      512 * (mapModeMain === "horizontal" ? 2 : 1) + "px";
    document.getElementById("fullowmainpanel").style.height =
      512 * (mapModeMain === "vertical" ? 2 : 1) + "px";
    document.getElementById("fullowmainpanelrow").style.height =
      512 * (mapModeMain === "vertical" ? 2 : 1) * fullZoomMain + "px";
    document.getElementById("fullowmainpanel").style.transform =
      "scale(" + fullZoomMain + ")";
  };

  window.applyFullOverworldZoomPopout = function () {
    document.getElementById("fullowpanel").style.width =
      512 * (mapModePopout === "horizontal" ? 2 : 1) + "px";
    document.getElementById("fullowpanel").style.height =
      512 * (mapModePopout === "vertical" ? 2 : 1) + "px";
    document.getElementById("fullowpanelrow").style.height =
      512 * (mapModePopout === "vertical" ? 2 : 1) * fullZoomPopout + "px";
    document.getElementById("fullowpanel").style.transform =
      "scale(" + fullZoomPopout + ")";
  };
  ...
  window.applyOverworldPathZoom = function () {
    document.getElementById("owpinnedpaths").style.transform =
      "scale(" + pathZoom + ")";
    document.getElementById("owpreviouspaths").style.transform =
      "scale(" + pathZoom + ")";
    document.getElementById("owsearchresults").style.transform =
      "scale(" + pathZoom + ")";
    document.getElementById("currentclickedowpath").style.transform =
      "scale(" + pathZoom + ")";
    document.getElementById("owpinnedpaths").style.height =
      (pinnedPaths.length * 66 - 2) * pathZoom + "px";
    document.getElementById("owpreviouspaths").style.height =
    ...
    if (pathZoom <= 0.5) {
      ...
      document
        .getElementById("zoomoutowpathprevious")
        .classList.add("disabled");
      document.getElementById("zoomoutowpathsearch").classList.add("disabled");
    } else {
      document
        .getElementById("zoomoutowpathpinned")
        .classList.remove("disabled");
      document
        .getElementById("zoomoutowpathprevious")
        .classList.remove("disabled");
      document
        .getElementById("zoomoutowpathsearch")
        .classList.remove("disabled");
    }
    if (pathZoom >= 2) {
      ...
      document
        .getElementById("zoominowpathpinned")
        .classList.remove("disabled");
      document
        .getElementById("zoominowpathprevious")
        .classList.remove("disabled");
      document
        .getElementById("zoominowpathsearch")
        .classList.remove("disabled");
    }
  };
  ...
  window.getModifiedOwnItems = function () {
    ...
    if (!document.getElementById("activeflutebox").checked) items.flute = false;
    if (
      !document.getElementById("globalsync").checked &&
      !document.getElementById("itemsync").checked
    ) {
    ...
    if (doorshuffle === "N") {
      ...
      items.connectorboehcb =
        items.connectorboehcc =
        items.connectorsanchcb =
        items.connectorsanchcc =
          false;
    }
    if (!items.gloves) {
      items.connectorboesanc =
        items.connectorboehcb =
        items.connectorboehcc =
          false;
    }
  ...
  window.updateReachableEdges = function () {
    ...
    for (let start of startRegions)
      if (
        !visitedRegions.has(start) &&
        (!mixedow || start.screen.mixedState !== "unknown")
      )
        explore(
          start,
          items,
        ...
        for (let id of cs) {
            ...
            if (cr.has(n + " normal"))
              for (let start of cr.get(n + " normal"))
                explore(
                  ...
                  regionsAssumeNormal,
                  edgesAssumeNormal,
                  checkableAssumeNormal,
                  continueAssumeNormal,
                  assumptions,
                );
          for (let n of group) assumptions.set(n, "swapped");
          for (let n of group)
            if (cr.has(n + " swapped"))
              for (let start of cr.get(n + " swapped"))
                explore(
                  ...
                  items,
                  options,
                  regionsAssumeSwapped,
                  edgesAssumeSwapped,
                  checkableAssumeSwapped,
                  continueAssumeSwapped,
                  assumptions,
                );
          ...
          if (!maybeMode)
            for (let c of commonCheckable) {
              ...
              let commonNormal = setIntersection(
                continueAssumeNormal.get(c + " normal"),
                continueAssumeSwapped.get(c + " normal"),
              );
              for (let region of commonNormal) crNormal.add(region);
              let commonSwapped = setIntersection(
                continueAssumeNormal.get(c + " swapped"),
                continueAssumeSwapped.get(c + " swapped"),
              );
          ...
          for (let c of maybeCheckable) {
            ...
            let normalSize = crNormal.size,
              swappedSize = crSwapped.size;
            let commonNormal = setUnion(
              continueAssumeNormal.get(c + " normal"),
              continueAssumeSwapped.get(c + " normal"),
            );
            for (let region of commonNormal) crNormal.add(region);
            let commonSwapped = setUnion(
              continueAssumeNormal.get(c + " swapped"),
              continueAssumeSwapped.get(c + " swapped"),
            );
            for (let region of commonSwapped) crSwapped.add(region);
          ...
          let commonRegions = setIntersection(
            regionsAssumeNormal,
            regionsAssumeSwapped,
          );
          let maybeRegions = setUnion(
            regionsAssumeNormal,
            regionsAssumeSwapped,
          );
          if (!maybeMode) {
    ...
    if (layoutshuffle !== "N" || whirlpoolshuffle || crossedow === "C") {
      ...
      for (let edge of visitedScreenEdges)
        if (!edge.out && isShuffledEdge(edge)) c++;
    ...
    if (
      ...
      window.opener.postMessage(
        determineLocationAvailability(visitedRegions, evr, mvrl, mvrd, items),
        "*",
      );
    }
  };
    ...
    if (mixedow) {
      if (screen.mixedState === "unknown") {
        ...
        let region = regionName
          ? screen.regions.get(regionName)
          : screen.fluteRegion;
        continueRegions
          .get(keyNormal)
          .add(ignoreSwaps || worldState !== "I" ? region : region.parallel);
        continueRegions
          .get(keySwapped)
          .add(ignoreSwaps || worldState === "I" ? region : region.parallel);
      } else {
        let region = regionName
          ? screen.regions.get(regionName)
          : screen.fluteRegion;
        (regionName ? fixedStartRegions : fluteSpotRegions).push(
          ignoreSwaps ||
            (screen.mixedState === "swapped") === (worldState === "I")
            ? region
            : region.parallel,
        );
      }
      ...
      let region = regionName
        ...
        : screen.fluteRegion;
      (regionName ? fixedStartRegions : fluteSpotRegions).push(
        ignoreSwaps || worldState !== "I" ? region : region.parallel,
      );
    }
  };
})(window);
