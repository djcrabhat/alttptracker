(function (window) {
  ...
  function enemizer_check(i) {
    ...
    if (melee() && items.hookshot && items.icerod && items.firerod)
      return "available";
    if (!melee_bow() && !rod() && !cane() && items.boomerang === 0)
      return "unavailable";
    if (i === 10) {
    ...
    switch (enemizer[i]) {
      ...
      case 3:
        ...
        break;
      case 4:
        if (items.sword > 0 || items.hammer || items.bow > 1)
          return "available";
        break;
      case 5:
        if (
          ...
          (items.sword > 0 ||
            items.hammer ||
            (items.bow > 1 && (items.firerod || items.icerod)) ||
            ((items.firerod || items.icerod) && items.bottle > 1) ||
            (items.bottle > 0 && items.magic))
        )
          return "available";
        break;
      case 6:
        ...
        break;
      case 7:
        if (items.sword > 0 || items.hammer || items.somaria || items.byrna)
          return "available";
        break;
      case 8:
        if (
          items.firerod ||
          (items.bombos &&
            (items.sword > 0 || (flags.swordmode === "S" && items.hammer)))
        )
          return "available";
        break;
      case 9:
        ...
        break;
      case 10:
        if (items.firerod && items.icerod && (items.hammer || items.sword > 0))
          return "available";
        break;
    }
    return "unavailable";
  ...
  function canReachOtherWorld() {
    //Can get from light => dark, or dark => light
    return (
      items.moonpearl &&
      (items.glove === 2 || (items.glove && items.hammer) || items.agahnim)
    );
  ...
  function maxKeys(dungeon) {
    return flags.doorshuffle === "C"
      ? 29
      : [0, 1, 1, 6, 1, 3, 1, 2, 3, 4, 4, 1, 2][dungeon]; //Note: This assumes Key Drop Shuffle is off in Basic
  }

  function door_enemizer_check(dungeon) {
    if (dungeon === 6) {
      var atticCell =
        (flags.doorshuffle === "C" ? items.bombfloor : items.bomb) &&
        (items.bigkey6 || !flags.wildbigkeys);
    ...
    if (dungeon >= 10)
      return items.sword || items.hammer || items.net || dungeon === 11
        ? "available"
        : "unavailable";
    return dungeon === 7 && (!items.hammer || items.glove == 0)
      ? "unavailable"
      : enemizer_check(dungeon);
  }

  window.doorCheck = function (
    dungeon,
    onlyDarkPossible,
    darkRoom,
    torchDarkRoom,
    ...
    if (flags.doorshuffle === "N") return null;
    var doorcheck = "available",
      bosscheck = onlyBunny ? "unavailable" : door_enemizer_check(dungeon),
      wildsmallkeys = flags.wildkeys || flags.gametype === "R";
    if (goal === "boss") doorcheck = bosscheck;
    if (doorcheck === "unavailable") return "unavailable"; //Can't beat boss
    ...
    if (
      ...
      !items["boss" + dungeon] &&
      bosscheck != "available" &&
      items["chest" + dungeon] == 1
    ) {
      if (
        bosscheck === "unavailable" &&
        (flags.ambrosia === "Y" ||
          (flags.wildmaps &&
            flags.wildcompasses &&
            (wildsmallkeys || maxKeys(dungeon) == 0) &&
            flags.wildbigkeys))
      )
    ...
    if (
      ...
      bosscheck != "available" &&
      (items["chest" + dungeon] == 1 ||
        (!items["chestknown" + dungeon] && items["chest" + dungeon] == 2))
    ) {
      if (
        bosscheck === "unavailable" &&
        items["chestknown" + dungeon] &&
        (flags.ambrosia === "Y" ||
          (flags.wildmaps &&
            flags.wildcompasses &&
            wildsmallkeys &&
    ...
    if (doorcheck === "available" && onlyBunny) doorcheck = "possible";
    if (
      doorcheck === "available" &&
      goal === "item" &&
      flags.doorshuffle === "C" &&
    ...
    if (
      ...
      flags.wildkeys &&
      flags.gametype != "R" &&
      items["smallkey" + dungeonAlt] < maxKeys(dungeon)
    )
      doorcheck = "possible"; //Could need more small keys
    if (
      doorcheck === "available" &&
      flags.wildbigkeys &&
      (dungeon <= 10 || flags.doorshuffle === "C") &&
      !items["bigkey" + dungeonAlt]
    )
    ...
    if (
      ...
      bosscheck != "available" &&
      flags.ambrosia === "N" &&
      ((!wildsmallkeys && maxKeys(dungeon) > 0) || !flags.wildbigkeys)
    )
  ...
  window.dungeonBoss = function (
    ...
    if (dungeonID === 11)
      return items.chest11
        ? dungeonChests(11, dungeonEntrances, dungeonEntrancesBunny)
        : "opened";
    ...
    for (var k = 0; k < dungeonEntranceCounts[dungeonID]; k++) {
      ...
      if (dungeonEntrances[k] !== "unavailable") {
        if (state !== "available") state = dungeonEntrances[k];
    ...
    switch (dungeonID) {
      case 0:
        state =
          flags.doorshuffle === "N"
            ? EPBoss()
            : doorCheck(0, false, true, true, ["hookshot", "bow"], "boss");
        break;
      case 1:
        if (flags.doorshuffle === "N") {
          var front = "unavailable",
            back = dungeonEntrances[3];
          if (
            dungeonEntrances[0] === "available" ||
            dungeonEntrances[1] === "available" ||
            dungeonEntrances[2] === "available"
          )
            front = "available";
          else if (
            dungeonEntrances[0] === "possible" ||
            dungeonEntrances[1] === "possible" ||
            dungeonEntrances[2] === "possible"
          )
            front = "possible";
        ...
        state =
          flags.doorshuffle === "N"
            ...
            : doorCheck(
                ...
                false,
                [
                  (!flags.wildkeys && flags.gametype != "R") ||
                  !flags.wildbigkeys
                    ? "firesource"
                    : "",
                  "kill",
        ...
        state =
          flags.doorshuffle === "N"
            ...
            : doorCheck(
                ...
                false,
                false,
                ["freezor", "hammer", "glove", "hookshot", "somaria", "bomb"],
                "boss",
              );
        break;
      case 8:
        state =
          flags.doorshuffle === "N"
            ...
            : doorCheck(
                ...
                true,
                false,
                ["hookshot", "firesource", "somaria", "bomb"],
                "boss",
              );
        break;
      case 9:
      ...
      case 10:
        if (
          (crystalCheck() < flags.ganonvulncount && flags.goals != "A") ||
          ((crystalCheck() < flags.opentowercount || !items.agahnim2) &&
            flags.goals != "F") ||
          (flags.goals === "A" && (!items.agahnim || !allDungeonCheck()))
        )
          return "unavailable";
        if (
          (flags.swordmode != "S" && items.sword < 2) ||
      ...
      case 12:
        ...
        state =
          flags.doorshuffle === "C"
            ? doorCheck(12, false, true, true, [], "boss")
            : CTBoss();
    }
    if (
      flags.doorshuffle !== "N" &&
      state === "available" &&
      (best === "possible" || !allAccessible)
    )
      return "possible";
    if (
      flags.doorshuffle !== "N" &&
  ...
  window.dungeonChests = function (
    ...
    if (bunny && flags.doorshuffle === "N") return "unavailable";
    ...
    switch (dungeonID) {
      ...
      case 9:
        state =
          flags.doorshuffle === "N"
            ? TRChests(...dungeonEntrances)
            : doorCheck(
                9,
                false,
                true,
                false,
                ["somaria", "firerod", "laserbridge"],
                "item",
                bunny,
              );
        break;
      ...
      case 11:
        if (flags.doorshuffle === "N") {
          var front = "unavailable",
            back = dungeonEntrances[4],
            sanc = dungeonEntrances[3];
          if (
            dungeonEntrances[0] === "available" ||
            dungeonEntrances[1] === "available" ||
            dungeonEntrances[2] === "available"
  ...
  window.entranceChests = function (entranceNames, dungeonID) {
    if (
      ...
      (dungeonID < 10 && !dungeons[dungeonID].is_beaten)
    ) {
      var entranceAvail = [];
      var entranceBunny = [];
      var found = false;
      nextEntrance: for (var i = 0; i < entranceNames.length; i++) {
        ...
        //special cases
        if (
          entranceNames[i] == "placeholder" &&
          dungeonID == 5 &&
          canReachOutcastEntrance()
        ) {
          entranceAvail.push("available");
        ...
        if (entranceNames[i] == "placeholder" && dungeonID == 11 && i == 3) {
          entranceAvail.push(
            flags.gametype != "I" &&
              (flags.gametype == "S" || flags.doorshuffle == "N")
              ? "available"
              : "possible",
          );
        ...
        if (
          ...
          (entrances[22].known_location === "sanc" ||
            ...
            (entrances[24].known_location === "sanc" &&
              items.boots &&
              items.agahnim) ||
            (entrances[13].known_location === "sanc" &&
              items.glove > 0 &&
              (flags.gametype != "I" ||
                (items.moonpearl && canReachOutcastEntrance()))) ||
      ...
      if (found) {
        ...
        if (dungeonID < 10) {
          document.getElementById("entranceBoss" + dungeonID).style.visibility =
            !dungeons[dungeonID].is_beaten && flags.overworldshuffle === "N"
              ? "visible"
              : "hidden";
          document.getElementById("entranceBoss" + dungeonID).style.background =
            ConvertBossToColor(
              dungeonBoss(dungeonID, entranceAvail, entranceBunny),
            );
        }
      } else {
        if (dungeonID < 10) {
  ...
  window.DPBoss = function (front = "available", back = "unavailable") {
    if (
      front != back &&
      flags.entrancemode === "N" &&
      (items.glove || flags.glitches != "N")
    ) {
      if (front === "available" || back === "available")
        front = back = "available";
      else front = back = "possible";
    }
    var dungeoncheck = enemizer_check(1);
    if (
      !items.bigkey1 ||
      dungeoncheck === "unavailable" ||
      back === "unavailable" ||
      (!items.firerod && !items.lantern)
    )
      return "unavailable";
    ...
    if (!flags.wildbigkeys) {
      ...
      if (
        (flags.wildkeys && items.smallkey1 === 0 && flags.gametype != "R") ||
        !items.boots
      )
        return "possible";
      if (!flags.wildkeys && flags.gametype != "R" && !items.boots)
        return "possible";
    }
    return dungeoncheck;
  };
  ...
  window.HeraBoss = function () {
    ...
    if (
      (flags.wildkeys && items.smallkey2 === 0 && flags.gametype != "R") ||
      (!items.lantern && !items.firerod)
    )
      return !items.flute &&
        !items.lantern &&
  ...
  window.SPBoss = function () {
    ...
    if (!items.flippers || (!items.mirror && flags.entrancemode === "N"))
      return "unavailable";
    var dungeoncheck = enemizer_check(4);
    if (
      !items.hammer ||
      !items.hookshot ||
      (items.smallkey4 === 0 && flags.gametype != "R")
    )
      return "unavailable";
    return dungeoncheck;
  };
  ...
  window.SWBoss = function (front = "available", back = "unavailable") {
    ...
    if (
      front != back &&
      flags.entrancemode === "N" &&
      (items.firerod || front === "unavailable")
    ) {
      if (front === "available" || back === "available")
        front = back = "available";
      else front = back = "possible";
    ...
    var keycheck =
      front === "available" ||
      flags.gametype === "R" ||
      (flags.wildkeys && items.smallkey5)
        ? "available"
        : front === "possible" || (!flags.wildkeys && back != "unavailable")
          ? "possible"
          : "unavailable";
    if (
      back === "unavailable" ||
  ...
  window.TTBoss = function () {
    ...
    if (!items.bomb && (flags.bossshuffle === "N" || enemizer[6] === 7))
      return "unavailable";
    if (
      !items.bomb &&
      dungeoncheck === "available" &&
      flags.bossshuffle != "N" &&
      enemizer[6] % 11 === 0
    )
      dungeoncheck = "possible";
  ...
  window.IPBoss = function () {
    ...
    if (!items.hammer || items.glove === 0 || dungeoncheck === "unavailable")
      return "unavailable";
    ...
    if (flags.wildkeys || flags.gametype === "R") {
      if (
        flags.gametype != "R" &&
        (items.smallkey7 === 0 || (items.smallkey7 === 1 && !items.somaria))
      )
        return "possible";
    } else {
  ...
  window.TRBoss = function (
    front = "available",
    middle = "unavailable",
    bigchest = "unavailable",
    back = "unavailable",
  ) {
    if (
      back != "unavailable" &&
      ...
      //More complicated with dark room navigation
      middle = back;
    }
    if (
      bigchest != "unavailable" &&
      middle != "available" &&
      (flags.entrancemode === "N" ||
        ((items.somaria || items.hookshot) &&
          (melee_bow() || items.firerod || cane())))
    ) {
      middle = bigchest;
    }
    if (
    ...
    if (
      ...
      middle =
        (flags.wildkeys || flags.gametype === "R") && items.smallkey9 === 4
          ? front
          : "possible";
    }
    if (
      (middle != "unavailable" || bigchest != "unavailable") &&
      back != "available" &&
      items.somaria &&
      items.lantern &&
      (items.bomb || items.boots) &&
    ...
    if (
      ...
      !items.smallkey9 ||
      !items.somaria ||
      (back === "unavailable" && !items.bomb && !items.boots) ||
      dungeoncheck === "unavailable"
    )
      return "unavailable";
    if (flags.wildkeys) {
    ...
    if (
      ...
      (!items.firerod ||
        front != "available" ||
        middle != "available" ||
        (bigchest != "available" && !flags.wildkeys && flags.gametype != "R") ||
        back != "available")
    )
      return "possible";
  ...
  window.TRFrontBoss = function (medcheck) {
    ...
    return dungeoncheck === "available" && medcheck === "available"
      ? items.lantern
        ? "available"
        : "darkavailable"
      : items.lantern
  ...
  window.TRBackBoss = function () {
    var dungeoncheck = enemizer_check(9);
    if (!items.bigkey9 || !items.somaria || dungeoncheck === "unavailable")
      return "unavailable";
    if (flags.wildkeys) {
  ...
  window.GTBoss = function () {
    ...
    if (
      !items.bigkey10 ||
      (items.bow < 2 && flags.enemyshuffle === "N") ||
      (!items.lantern && !items.firerod) ||
      !items.hookshot ||
  ...
  window.CTBoss = function () {
    ...
    if (flags.wildkeys && flags.gametype != "R" && items.smallkeyhalf1 < 2)
      return "unavailable";
    return items.lantern ? "available" : "darkavailable";
  };
  ...
  window.EPChests = function () {
    ...
    //Big Key Chest
    if (!flags.wildbigkeys) {
      chests[4] = items.lantern
        ? "A"
        : (items.bow > 1 || flags.enemyshuffle != "N") &&
            dungeoncheck === "available"
          ? "DA"
          : "P";
    } else {
  ...
  window.HeraChests = function () {
    ...
    if (flags.wildbigkeys) {
      if (
        (items.smallkey2 === 0 && flags.gametype != "R") ||
        (!items.lantern && !items.firerod)
      ) {
        chests[2] = "U";
      } else {
    ...
    if (flags.wildbigkeys) {
      ...
      if (
        (items.smallkey2 === 1 || flags.gametype === "R") &&
        (items.lantern || items.firerod)
      ) {
  ...
  window.PoDChests = function () {
    ...
    if (!flags.wildbigkeys && !flags.wildkeys && flags.gametype != "R") {
      ...
      if ((items.bow > 1 || flags.enemyshuffle === "N") && items.bomb) {
        ...
        //Big Chest
        chests[12] =
          items.bigkey3 && items.bomb ? (items.lantern ? "P" : "DP") : "U";
        //Boss
        chests[13] = "U";
      }
      //5) Small Key + Big Key shuffle
      ...
      if (
        ...
        //Big Chest
        chests[12] =
          items.bigkey3 && items.bomb ? (items.lantern ? "A" : "DA") : "U";
      }
  ...
  window.SWChests = function (front = "available", back = "unavailable") {
    ...
    if (back != "unavailable") {
      ...
      chests[6] =
        back === "available"
          ? front != "available" && !flags.wildkeys && flags.gametype != "R"
            ? "P"
            : "A"
          : "P";
  ...
  window.TTChests = function () {
    ...
    if (items.bigkey6) {
      ...
      if (flags.wildbigkeys || flags.wildkeys || flags.gametype === "R") {
        chests[6] =
          (items.smallkey6 === 1 || flags.gametype == "R") && items.hammer
            ? "A"
            : "U";
      } else {
  ...
  window.IPChests = function () {
    ...
    if (items.bomb) {
      ...
      if (flags.wildkeys) {
        chests[1] =
          items.hookshot || items.smallkey7 > 0 || flags.gametype == "R"
            ? "A"
            : "U";
      } else {
  ...
  window.TRChests = function (
    ...
    if (
      middle === "available" &&
      bigchest === "available" &&
      TRMidChests().endsWith("available")
    )
      return TRMidChests();
    if (
      middle != "unavailable" &&
  ...
  window.TRMidChests = function () {
    ...
    if (!flags.wildbigkeys && !flags.wildkeys && flags.gametype != "R") {
      ...
      chests[5] =
        items.somaria || items.hookshot
          ? items.somaria && items.firerod
            ? "K"
            : "P"
          : "U"; //Reserved as big key, could be in the front of the dungeon
      ...
      //If you do not have somaria, you can get through with the big key
      chests[6] =
        items.somaria && items.firerod && items.smallkey9 > 0 ? "A" : "P";
  ...
  window.TRBackChests = function () {
    ...
    if (!flags.wildbigkeys && !flags.wildkeys && flags.gametype != "R") {
      ...
      if (flags.wildbigkeys) {
        chests[5] =
          items.bigkey9 && (items.somaria || items.hookshot) ? "A" : "U";
      } else {
        chests[5] =
          items.somaria || items.hookshot
            ? items.somaria && items.firerod
  ...
  window.GTChests = function () {
    ...
    if (!flags.wildbigkeys && !flags.wildkeys && flags.gametype != "R") {
      ...
      if (
        (!flags.wildbigkeys || items.bigkey10) &&
        (items.bow > 1 || flags.enemyshuffle != "N") &&
        (items.lantern || items.firerod)
      ) {
        //Mini Helmasaur Room - Left - 3
      ...
      if (
        ...
        if (items.bomb) {
          //Pre-Moldorm Chest - 3
          chests[25] =
            items.smallkey10 > 2 || flags.gametype == "R" ? "A" : "P";
      ...
      if (
        (items.bow > 1 || flags.enemyshuffle != "N") &&
        (items.lantern || items.firerod) &&
        (items.smallkey10 > 0 || flags.entrancemode != "Y") &&
        items.bigkey10
      ) {
        //Mini Helmasaur Room - Left - 3
        chests[23] =
        ...
        if (items.bomb) {
          ...
          chests[25] =
            items.smallkey10 > 2 ||
            flags.gametype == "R" ||
            (flags.entrancemode != "Y" && items.smallkey10 > 0)
              ? "A"
              : "P";
          ...
          if (items.hookshot) {
            ...
            chests[26] =
              items.smallkey10 > 2 ||
              flags.gametype == "R" ||
              (flags.entrancemode != "Y" && items.smallkey10 > 1)
                ? "A"
                : "P";
          }
        }
  ...
  window.HCChests = function (
    ...
    if (
      ...
      back != "available" &&
      canDoTorchDarkRooms() &&
      (weapon || items.icerod || flags.gametype === "R")
    ) {
      //Could consider dark room navigation
      if (flags.gametype === "R") {
        back = front;
      } else {
        if (!flags.wildkeys) {
    ...
    if (
      ...
      back === "unavailable" &&
      (weapon || items.icerod || flags.gametype === "R") &&
      (flags.gametype === "R" || !flags.wildkeys || items.smallkeyhalf0) &&
      (items.bomb || items.boots)
    ) {
      chests[4] =
        front === "available" &&
    ...
    if (!flags.wildkeys && flags.gametype != "R") {
      if (flags.gametype === "S") {
        ...
        for (var k = 0; k < 8; k++) {
          //Small key could be anywhere. Temporary bad solution
          if (chests[k] === "A") {
            chests[k] = "P";
            break;
          }
  ...
  window.CTChests = function () {
    ...
    if (flags.wildkeys || flags.gametype === "R") {
      chests[0] = "A";
      if (items.smallkeyhalf1 > 0) chests[1] = items.lantern ? "A" : "DA";
    } else {
      chests[0] = "K";
      chests[1] = "K";
    }
