export function start_tracker() {
  ...
  if (flags.colormode === "Y") {
    document.getElementById("alternatestyle").href =
      "css/alternatecolorstyle.css?v=" + buildString;
  }
  ...
  for (const a of [
    ...
    for (var k = 0; k < 13; k++) {
      ...
      constantFunctionsEDC[a].push(function () {
        if (a === "available") {
          document.getElementById("chest" + dungeonID).style.backgroundColor =
            flags.colormode === "N" ? "lime" : "#055fe6";
          document.getElementById("chest" + dungeonID).style.color = "black";
        } else if (a === "darkavailable") {
          document.getElementById("chest" + dungeonID).style.backgroundColor =
            flags.colormode === "N" ? "blue" : "#034f6d";
          document.getElementById("chest" + dungeonID).style.color = "white";
        } else if (a === "possible") {
          document.getElementById("chest" + dungeonID).style.backgroundColor =
            flags.colormode === "N" ? "yellow" : "#faf600";
          document.getElementById("chest" + dungeonID).style.color = "black";
        } else if (a === "darkpossible") {
          document.getElementById("chest" + dungeonID).style.backgroundColor =
            flags.colormode === "N" ? "purple" : "#cdbd00";
          document.getElementById("chest" + dungeonID).style.color = "white";
        } else if (a === "unavailable") {
          document.getElementById("chest" + dungeonID).style.backgroundColor =
            flags.colormode === "N" ? "red" : "#c46902";
          document.getElementById("chest" + dungeonID).style.color = "white";
        } else if (a === "information") {
          document.getElementById("chest" + dungeonID).style.backgroundColor =
            flags.colormode === "N" ? "orange" : "#d53ae6";
          document.getElementById("chest" + dungeonID).style.color = "black";
        }
      });
    }
...
(function (window) {
  ...
  window.sendTrackerCommand = function () {
    if (flags.restreamer === "T") {
      ...
      xhr.open(
        "POST",
        "../api/v1/RestreamerAPI/PostItemObject?code=" + flags.restreamingcode,
        true,
      );
  ...
  window.getTrackerCommand = function () {
    ...
    xhr.open(
      "GET",
      "../api/v1/RestreamerAPI/GetItemObject?code=" +
        flags.restreamingcode +
        "&delay=" +
        flags.restreamdelay +
        "&guid=" +
        restreamingguid,
  ...
  window.resetClasses = function (label, value) {
    ...
    for (var i = 0; i < 10; i++) {
      document.getElementById("dungeonPrize" + i).className =
        "prize-" + prizes[i];
    }

    for (var i = 0; i < 2; i++) {
      document.getElementById("medallion" + i).className =
    ...
    if (label.substring(0, 5) === "chest") {
      if (value === 0) {
        ...
        document.getElementById(label).innerHTML =
          flags.doorshuffle === "C" && !items["chestknown" + label.substring(5)]
            ? value - 1 + "+"
            : value;
      }
    }
    ...
    if (label.substring(0, 6) === "bigkey") {
      if (items[label]) {
        document.getElementById(label).className =
          label.substring(0, 10) == "bigkeyhalf"
            ? "bigkeyhalf collected"
            : "bigkey collected";
      } else {
        document.getElementById(label).className =
          label.substring(0, 10) == "bigkeyhalf" ? "bigkeyhalf" : "bigkey";
      }
    }
    ...
    if (typeof value === "boolean") {
      nodes.forEach((node) =>
        node.classList[items[label] ? "add" : "remove"](
          isboss ? "defeated" : "active",
        ),
      );
    } else {
    ...
    if (label === "moonpearl" || label === "tunic") {
      document
        .getElementsByClassName("tunic")[0]
        .classList[!items.moonpearl ? "add" : "remove"]("bunny");
    }
  ...
  window.toggle = function (label) {
    if (rightClickedLocation != -1) {
      ...
      if (rightClickedType === "chest") {
        if (name.charAt(0) < "a" || name.charAt(0) > "z") {
          ...
          document.getElementById("caption").innerHTML = caption_to_html(
            name + " placed at " + chests[rightClickedLocation].caption,
          );
        }
        document
          .getElementById("locationMap" + rightClickedLocation)
          .classList.remove("rightclick");
      }
      if (rightClickedType === "dungeon") {
        if (name.charAt(0) < "a" || name.charAt(0) > "z") {
          ...
          document.getElementById("caption").innerHTML = caption_to_html(
            name + " placed in " + dungeons[rightClickedLocation].caption,
          );
        }
        document
          .getElementById("dungeon" + rightClickedLocation)
          .classList.remove("rightclick");
      }
    ...
    if (label === "mirror" && flags.doorshuffle != "N") {
      document.getElementById("mirrorscroll").style.display = items.mirror
        ? "block"
        : "none";
    }
    ...
    if (label.substring(0, 5) === "chest") {
      ...
      if (flags.mapmode != "N") {
        if (flags.entrancemode === "N") {
          ...
          document.getElementById("dungeon" + x).className =
            "dungeon " + (value ? dungeons[x].can_get_chest() : "opened");
          if (label == "chest11") {
            document.getElementById("bossMap11").className =
              "bossprize-" + prizes[11] + " boss " + dungeons[11].is_beatable();
          }
        } else {
    ...
    if (
      label.substring(0, 8) === "smallkey" &&
      label.substring(0, 12) != "smallkeyhalf"
    ) {
      if (flags.gametype != "R") {
        var value = items.inc(label);
        document.getElementById(label).innerHTML = value;
    ...
    if (!skipkey) {
      ...
      if (typeof items[label] === "boolean") {
        ...
        if (items[label] == true) lastItem = label;
        else lastItem = null;
        if (label === "mushroom") {
        ...
        if (label === "sword" && flags.swordmode === "S") {
          ...
          if (
            label === "bow" &&
            value === 1 &&
            window.flags.nonprogressivebows === false
          )
    ...
    if (flags.mapmode != "N") {
      for (var k = 0; k < chests.length; k++) {
        if (!chests[k].is_opened)
          document.getElementById("locationMap" + k).className =
            "location " + chests[k].is_available();
      }
      if (flags.entrancemode != "N") {
          ...
          if (!entrances[k].is_opened) {
            ...
            if (entrances[k].is_available()) {
              if (entrances[k].known_location != "") {
                entrancetype = isDungeon(entrances[k].known_location)
                  ? "dungeon"
                  : "keylocation";
            ...
            ent.className =
              "entrance " +
              entrances[k].is_available() +
              entrancetype +
              (ent.classList.contains("highlight") ? " highlight" : "");
          }
        }
        ...
        for (var k = 0; k < dungeons.length; k++) {
          document.getElementById("bossMap" + k).className =
            "bossprize-" +
            prizes[k] +
            " boss " +
            (dungeons[k].is_beaten ? "opened" : dungeons[k].is_beatable());
          if (items["chest" + k])
            document.getElementById("dungeon" + k).className =
              "dungeon " + dungeons[k].can_get_chest();
        }
      }
      toggle_agahnim();
    }
      ...
      if (event.data.logic && flags.overworldshuffle != "N") {
        ...
        if (
          ...
          doorCheck(
            ...
            false,
            [
              (!flags.wildkeys && flags.gametype != "R") || !flags.wildbigkeys
                ? "boots"
                : "",
              "firesource",
              "killbomb",
          ...
          event.data.items[48] =
            event.data.items[48] === "darkpossible"
              ? "darkavailable"
              : "available";
        if (
          event.data.helpMimic &&
          doorCheck(
            9,
            false,
            true,
          ...
          event.data.items[4] =
            event.data.items[4] === "darkpossible"
              ? "darkavailable"
              : "available";
        for (var k = 0; k < chests.length; k++)
          chests[k].is_available =
            constantFunctions[
              event.data[
                flags.entrancemode === "N" ? "items" : "entranceitems"
              ][k]
            ];
        for (var k = 0; k < dungeons.length; k++) {
          ...
          var bossAvail = dungeonBoss(
            k,
            dungeonEntrances,
            dungeonEntrancesBunny,
          );
          var chestsAvail = dungeonChests(
            k,
            dungeonEntrances,
            dungeonEntrancesBunny,
          );
          ...
          if (flags.entrancemode != "N") {
            ...
            dungeonChecks[k].can_get_chest = function () {
              ...
              document.getElementById("chest" + dungeonID).style.color =
                "black";
            };
          }
        }
        ...
        if (flags.entrancemode != "N") {
          for (var k = 0; k < entrances.length; k++)
            entrances[k].is_available =
              constantFunctions[event.data.entrances[k]];
        }
        updateLocationAvailability();
        ...
        event.data == "RESETLOGIC" &&
        flags.overworldshuffle != "N" &&
        doorWindow &&
        !doorWindow.closed
      ) {
  ...
  window.showDoorWindow = function () {
    if (doorWindow && !doorWindow.closed) doorWindow.focus();
    else {
      var url =
        "dungeontracker.html?door_shuffle=" +
        flags.doorshuffle +
        "&overworld_shuffle=" +
        flags.overworldshuffle;
      url +=
        "&wild_keys=" +
        flags.wildkeys +
        "&wild_big_keys=" +
      ...
      url +=
        ...
        "&t=" +
        Date.now();
      doorWindow = window.open(
        url,
        "",
        "width=444,height=720,titlebar=0,menubar=0,toolbar=0,scrollbars=1,resizable=1",
      );
    }
  };
  ...
  window.resetChestsKeepTrackedData = function () {
    ...
    flags.entrancemode === "N"
      ? loadChestFlagsItem()
      : loadChestFlagsEntrance();
    for (var k = 0; k < dungeons.length; k++) {
  ...
  window.toggle_dungeon = function (n) {
    ...
    document.getElementById("dungeonPrize" + n).className =
      "prize-" + prizes[n];
  ...
  window.rightClickEnemy = function (n) {
    ...
    document.getElementById("dungeonEnemy" + n).className =
      "enemizer-" + enemizer[n];
    dungeons[n].is_beatable();
    if (!dungeons[n].is_beaten)
      if (document.getElementById("bossMap" + n) != null) {
        document.getElementById("bossMap" + n).className =
          "bossprize-" + prizes[n] + " boss " + dungeons[n].is_beatable();
      }
  ...
  window.clickCompass = function (dungeonid) {
    ...
    document.getElementById("chest" + dungeonid).innerHTML =
      items["chest" + dungeonid] == 0
        ? ""
        : flags.doorshuffle === "C" && !items["chestknown" + dungeonid]
          ? items["chest" + dungeonid] - 1 + "+"
          : items["chest" + dungeonid];
    updateMapTracker();
  };
  ...
  window.toggle_bomb_floor = function () {
    ...
    document.getElementById("bombfloor").className =
      "bombfloor-" + (items.bombfloor ? 1 : 0);
  ...
  window.rightClickLocation = function (n) {
    if (rightClickedLocation === -1) {
      ...
      document.getElementById("caption").innerHTML = caption_to_html(
        "Select an item to place at " + chests[rightClickedLocation].caption,
      );
    } else if (rightClickedType === "chest" && rightClickedLocation === n) {
      chests[n].content = "";
      document.getElementById("caption").innerHTML = caption_to_html(
        "Content of " + chests[rightClickedLocation].caption + " cleared",
      );
  ...
  window.rightClickDungeon = function (n) {
    if (rightClickedLocation === -1) {
      ...
      document.getElementById("caption").innerHTML = caption_to_html(
        "Select an item to place in " + dungeons[rightClickedLocation].caption,
      );
    } else if (rightClickedType === "dungeon" && rightClickedLocation === n) {
      dungeons[n].content = "";
      document.getElementById("caption").innerHTML = caption_to_html(
        "Content of " + dungeons[rightClickedLocation].caption + " cleared",
      );
  ...
  window.rightClickEntrance = function (n) {
    ...
    document.getElementById("entranceModalTitle").innerHTML = entrances[
      n
    ].caption.replace(/\s?\{[^}]+\}/g, "");
    ...
    if (entrances[n].is_connector) {
      for (var i = 0; i < connectorIndex.length; i++) {
        if (
          (connectorOne[i] === n || connectorTwo[i] === n) &&
          entrancecount < 3
        ) {
          ...
          spanclone.setAttribute(
            "onClick",
            "entranceDisconnect(" + connectorIndex[i] + "," + n + ");",
          );
          spanclone.style.visibility = "visible";
          if (connectorOne[i] === n) {
            spanclone.innerHTML =
              entrances[connectorTwo[i]].caption +
              '&nbsp;&nbsp;&nbsp;<img style="height: 15px;"src="./images/interface/cancel.png" />&nbsp;&nbsp;&nbsp;';
          } else {
            spanclone.innerHTML =
              entrances[connectorOne[i]].caption +
              '&nbsp;&nbsp;&nbsp;<img style="height: 15px;"src="./images/interface/cancel.png" />&nbsp;&nbsp;&nbsp;';
          }
  ...
  window.hideEntranceModal = function () {
    if (overrideEntranceCloseFlag === false) {
      if (document.getElementById("entranceModal").style.display != "none") {
        entrances[document.getElementById("entranceID").value].note =
          document.getElementById("entranceModalNote").value;
        if (document.getElementById("entranceModalNote").value != "") {
          ...
          var loc = document.getElementById(
            "entranceMap" + document.getElementById("entranceID").value,
          );
          ...
          var divtoremove = document.getElementById(
            "notediv" + document.getElementById("entranceID").value,
          );
          if (divtoremove != null) {
            divtoremove.remove();
          }
  ...
  window.HideConnectors = function () {
    if (
      document.getElementById("connectorLineDiv").style.visibility ===
      "collapse"
    ) {
      document.getElementById("connectorLineDiv").style.visibility = "visible";
      document.getElementById("hideConnectorLinesImg").src =
        "./images/interface/hide.png";
    } else {
      document.getElementById("connectorLineDiv").style.visibility = "collapse";
      document.getElementById("hideConnectorLinesImg").src =
        "./images/interface/show.png";
    }
  };
  ...
  window.LoadEntranceSummary = function (index = -1) {
    ...
    for (var i = 0; i < 2; i++) {
      if (index < 0 || i === index) {
        var includeCleared = document.getElementById(
          "summaryCleared" + i,
        ).checked;
        ...
        switch (document.getElementById("summaryFilter" + i).value) {
          case "knownconnectors":
            ...
            for (var j = 0; j < connectorIndex.length; j++) {
              if (
                includeCleared ||
                !entrances[connectorOne[j]].is_opened ||
                !entrances[connectorTwo[j]].is_opened
              ) {
                entrancesummary +=
                  "<div>" +
                  entrances[connectorOne[j]].caption +
                  " <--> " +
                  entrances[connectorTwo[j]].caption +
                  "</div>";
              }
            }

            document.getElementById("summaryDiv" + i).innerHTML =
              entrancesummary.replace(/\s?\{[^}]+\}/g, "");
            break;
          default:
            var locations = [];
            var locationsummary = "";
            ...
            for (var j = 0; j < entrances.length; j++) {
              if (
                (entrances[j].known_location != "" ||
                  entrances[j].note != "") &&
                (includeCleared || !entrances[j].is_opened)
              ) {
                switch (document.getElementById("summaryFilter" + i).value) {
                  case "all":
                  ...
                  case "starts":
                    if (
                      entranceNameToGroup[entrances[j].known_location] ===
                      "start"
                    ) {
                      pushLocationObject(locations, entrances[j]);
                    }
                  ...
                  case "keylocations":
                    if (
                      entranceNameToGroup[entrances[j].known_location].endsWith(
                        "key",
                      )
                    ) {
                      pushLocationObject(locations, entrances[j]);
                  ...
                  case "shopschests":
                    if (
                      entrances[j].known_location === "magic" ||
                      entrances[j].known_location === "bomb" ||
                      entrances[j].known_location === "shop" ||
                      entrances[j].known_location === "chest"
                    ) {
                      pushLocationObject(locations, entrances[j]);
                    }
                    break;
                  case "unknownconnectors":
                    if (
                      entrances[j].known_location === "dark" ||
                      entrances[j].known_location === "connector"
                    ) {
                      pushLocationObject(locations, entrances[j]);
                    }
                    break;
            ...
            for (var j = 0; j < locations.length; j++) {
              locationsummary +=
                (lastGroup != "" && lastGroup != locations[j].group
  ...
  window.tagEntrance = function (n, t) {
    ...
    if (
      ...
      var information = document.getElementById(
        "informationdiv" + document.getElementById("entranceID").value,
      );
      if (information != null) {
        information.remove();
      }
      ...
      entrances[document.getElementById("entranceID").value].type =
        t === true ? 2 : 3;
      ...
      if (
        ...
        document.getElementById(
          "informationdiv" + document.getElementById("entranceID").value,
        ).innerHTML = n.replace("_", "-").toUpperCase();
      } else {
        var divtoadd = document.createElement("div");
        divtoadd.id =
          "informationdiv" + document.getElementById("entranceID").value;
  ...
  window.toggle_medallion = function (n) {
    ...
    document.getElementById("medallion" + n).className =
      "medallion-" + medallions[n];

    if (flags.mapmode != "N") {
      // Change the mouseover text on the map
      dungeons[8 + n].caption = dungeons[8 + n].caption.replace(
        /\{medallion\d+\}/,
        "{medallion" + medallions[n] + "}",
      );
      updateMapTracker();
  ...
  window.toggle_bigkey = function (n) {
    ...
    if (flags.mapmode != "N") {
      ...
      if (items["chest" + (8 + n)] > 0)
        document.getElementById("dungeon" + (8 + n)).className =
          "dungeon " + dungeons[8 + n].can_get_chest();
  ...
  window.toggle_chest = function (x) {
    ...
    var highlight = document
      .getElementById("locationMap" + x)
      .classList.contains("highlight");
    document.getElementById("locationMap" + x).className =
      "location " +
  ...
  window.toggle_location = function (x) {
    if (connectStart === false) {
      ...
      var highlight = document
        .getElementById("entranceMap" + x)
        .classList.contains("highlight");
      document.getElementById("entranceMap" + x).className =
        "entrance " +
        (entrances[x].is_opened ? "opened" : entrances[x].is_available()) +
        (highlight ? " highlight" : "");
      var information = document.getElementById("informationdiv" + x);
      if (information != null) {
        information.style.visibility = entrances[x].is_opened
          ? "collapse"
          : "visible";
      }
      ...
      if (x != parseInt(document.getElementById("entranceID").value)) {
        ...
        entrances[document.getElementById("entranceID").value].is_connector =
          true;

        connectorIndex.push(connectorid);
        connectorOne.push(
          parseInt(document.getElementById("entranceID").value),
        );
        connectorTwo.push(x);
        ...
        var connector2 = document.getElementById(
          "entranceMap" + document.getElementById("entranceID").value,
        );

        if (connector1.offsetTop > connector2.offsetTop) {
          divtoadd.style.top =
            connector2.offsetTop + (flags.mapmode === "C" ? 4.5 : 6);
        } else {
          divtoadd.style.top =
            connector1.offsetTop + (flags.mapmode === "C" ? 4.5 : 6);
        }
        if (connector1.offsetLeft > connector2.offsetLeft) {
          divtoadd.style.left =
            connector2.offsetLeft + (flags.mapmode === "C" ? 4.5 : 6);
        } else {
          divtoadd.style.left =
            connector1.offsetLeft + (flags.mapmode === "C" ? 4.5 : 6);
        }
        ...
        divtoadd.style.width = Math.abs(
          connector1.offsetLeft - connector2.offsetLeft,
        );
        divtoadd.style.height = Math.abs(
          connector1.offsetTop - connector2.offsetTop,
        );
        divtoadd.style.position = "absolute";
  ...
  window.toggle_boss = function (x) {
    ...
    if (document.getElementById("bossMap" + x) != null) {
      document.getElementById("bossMap" + x).className =
        "bossprize-" +
        prizes[x] +
        " boss " +
  ...
  window.toggle_agahnim = function () {
    if (flags.entrancemode === "N") {
      document.getElementById("castle").className =
        "castle " + (items.agahnim ? "opened" : agahnim.is_available());
    }
  ...
  window.highlight = function (x) {
    ...
    document.getElementById("caption").innerHTML = caption_to_html(
      chests[x].content
        ? chests[x].content + " | " + chests[x].caption
        : chests[x].caption,
    );
  ...
  window.highlight_entrance = function (x) {
    ...
    if (entrances[x].known_location != "") {
      displayCaption =
        displayCaption + " -- " + getFriendlyName(entrances[x].known_location);
    }
    if (entrances[x].is_connector) {
      for (var i = 0; i < connectorIndex.length; i++) {
        if (connectorOne[i] === x) {
          displayCaption =
            displayCaption + " ==> " + entrances[connectorTwo[i]].caption;
        }
        if (connectorTwo[i] === x) {
          displayCaption =
            displayCaption + " ==> " + entrances[connectorOne[i]].caption;
        }
      }
    }
    if (entrances[x].note != "") {
    ...
    document.getElementById("caption").innerHTML =
      caption_to_html(displayCaption);
    document.getElementById("autotrackingstatus").style.display = "none";
  };
  ...
  window.highlight_dungeon = function (x) {
    ...
    document.getElementById("caption").innerHTML = caption_to_html(
      (dungeons[x].content ? dungeons[x].content + " | " : "") +
        (dungeons[x].trashContent ? dungeons[x].trashContent + " | " : "") +
        dungeons[x].caption,
    );
    document.getElementById("autotrackingstatus").style.display = "none";
  };
      ...
      for (var i = 0; i < dungeonContents.length; i++) {
        ...
        for (var locationName in itemMap) {
          ...
          if (hasItem) {
            results =
              results === ""
                ? dungeonNames[i] + " " + locationName
                : results + ", " + dungeonNames[i] + " " + locationName;
          }
        }
        if (dungeonHasItem) {
          if (flags.mapmode != "N")
  ...
  window.setSphereItem = function (label) {
    if (lastItem === null) {
      ...
      if (
        lastItem.substring(0, 5) === "sword" ||
        lastItem.substring(0, 5) === "shiel" ||
        lastItem.substring(0, 5) === "moonp"
      ) {
  ...
  function caption_to_html(caption) {
    return caption.replace(/\{(\w+?)(\d+)?\}/g, function (__, name, n) {
      ...
      return (
        '<div class="icon ' +
        (dash ? name + "-" + n : n ? name + " active-" + n : name) +
        '"></div>'
      );
    });
  }
  ...
  window.changeFlags = function () {
    ...
    document.getElementById("activatedflute").checked = flags.activatedflute
      ? true
      : false;
    document.getElementById("doorselect").value = flags.doorshuffle;
    document.getElementById("overworldbox").checked =
      flags.overworldshuffle != "N";
  ...
  window.adjustFlags = function () {
    ...
    if (document.getElementById("stateselect").value != flags.gametype) {
      if (
        document.getElementById("stateselect").value === "R" ||
        flags.gametype === "R"
      ) {
        adjustForRetro = true;
      }
      ...
      if (
        document.getElementById("stateselect").value === "I" ||
        flags.gametype === "I"
      ) {
    ...
    if (document.getElementById("bossselect").value != flags.bossshuffle) {
      ...
      if (flags.bossshuffle === "N") {
        ...
        for (var k = 0; k < dungeons.length; k++) {
          enemizer[k] = k + 1;
          if (k < 10) {
            document.getElementById("dungeonEnemy" + k).className =
              "enemizer-" + (k + 1);
          }
        }
      } else {
    ...
    if (
      document.getElementById("shuffledmaps").checked != flags.wildmaps ||
      document.getElementById("shuffledcompasses").checked !=
        flags.wildcompasses ||
      document.getElementById("shuffledkeys").checked != flags.wildkeys ||
      document.getElementById("shuffledbigkeys").checked != flags.wildbigkeys ||
      document.getElementById("doorselect").value != flags.doorshuffle ||
      ...
      var adjustForCrossed =
        (document.getElementById("doorselect").value === "C") !=
        (flags.doorshuffle === "C");

      var chestschecked0 = adjustForCrossed
        ? 0
        : items.maxchest0 - items.chest0;
      var chestschecked1 = adjustForCrossed
        ? 0
        : items.maxchest1 - items.chest1;
      var chestschecked2 = adjustForCrossed
        ? 0
        : items.maxchest2 - items.chest2;
      var chestschecked3 = adjustForCrossed
        ? 0
        : items.maxchest3 - items.chest3;
      var chestschecked4 = adjustForCrossed
        ? 0
        : items.maxchest4 - items.chest4;
      var chestschecked5 = adjustForCrossed
        ? 0
        : items.maxchest5 - items.chest5;
      var chestschecked6 = adjustForCrossed
        ? 0
        : items.maxchest6 - items.chest6;
      var chestschecked7 = adjustForCrossed
        ? 0
        : items.maxchest7 - items.chest7;
      var chestschecked8 = adjustForCrossed
        ? 0
        : items.maxchest8 - items.chest8;
      var chestschecked9 = adjustForCrossed
        ? 0
        : items.maxchest9 - items.chest9;
      var chestschecked10 = adjustForCrossed
        ? 0
        : items.maxchest10 - items.chest10;
      var chestschecked11 = adjustForCrossed
        ? 0
        : items.maxchest11 - items.chest11;
      var chestschecked12 = adjustForCrossed
        ? 0
        : items.maxchest12 - items.chest12;
      ...
      if (document.getElementById("shuffledbigkeys").checked) {
        chestmod++;
        if (
          document.getElementById("shuffledkeys").checked ||
          flags.gametype === "R"
        ) {
          chestmodcrossed++;
        }
      ...
      var chests11 =
        document.getElementById("doorselect").value === "C"
          ? 3 + chestmodcrossed
          : 6 +
            (document.getElementById("shuffledmaps").checked ? 1 : 0) +
            (document.getElementById("shuffledkeys").checked ||
            flags.gametype === "R"
              ? 1
              : 0);
      ...
      var maxchests0 =
        document.getElementById("doorselect").value === "C" ? 32 : chests0;
      var maxchests1 =
        document.getElementById("doorselect").value === "C" ? 32 : chests1;
      var maxchests2 =
        document.getElementById("doorselect").value === "C" ? 32 : chests2;
      var maxchests3 =
        document.getElementById("doorselect").value === "C" ? 32 : chests3;
      var maxchests4 =
        document.getElementById("doorselect").value === "C" ? 32 : chests4;
      var maxchests5 =
        document.getElementById("doorselect").value === "C" ? 32 : chests5;
      var maxchests6 =
        document.getElementById("doorselect").value === "C" ? 32 : chests6;
      var maxchests7 =
        document.getElementById("doorselect").value === "C" ? 32 : chests7;
      var maxchests8 =
        document.getElementById("doorselect").value === "C" ? 32 : chests8;
      var maxchests9 =
        document.getElementById("doorselect").value === "C" ? 32 : chests9;
      var maxchests10 =
        document.getElementById("doorselect").value === "C" ? 32 : chests10;
      var maxchests11 =
        document.getElementById("doorselect").value === "C" ? 32 : chests11;
      var maxchests12 =
        document.getElementById("doorselect").value === "C" ? 32 : chests12;
      ...
      items.inc = limit(1, {
        ...
        smallkey0: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 0,
        },
        smallkey1: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 1,
        },
        smallkey2: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 1,
        ...
        smallkey3: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 6,
        },
        smallkey4: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 1,
        ...
        smallkey5: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 3,
        },
        smallkey6: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 1,
        ...
        smallkey7: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 2,
        },
        smallkey8: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 3,
        ...
        smallkey9: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 4,
        },
        smallkey10: {
          min: 0,
          max: document.getElementById("doorselect").value === "C" ? 29 : 4,
      ...
      if (!document.getElementById("shuffledbigkeys").checked) {
        ...
        document.getElementById("bigkeyhalf0").style.visibility =
          document.getElementById("doorselect").value === "C"
            ? "visible"
            : "hidden";
        document.getElementById("bigkeyhalf1").style.visibility =
          document.getElementById("doorselect").value === "C"
            ? "visible"
            : "hidden";
      }
      ...
      if (
        ...
        items.smallkey0 =
          document.getElementById("doorselect").value === "C" ? 29 : 0;
        items.smallkey1 =
          document.getElementById("doorselect").value === "C" ? 29 : 1;
        items.smallkey2 =
          document.getElementById("doorselect").value === "C" ? 29 : 1;
        items.smallkey3 =
          document.getElementById("doorselect").value === "C" ? 29 : 6;
        items.smallkey4 =
          document.getElementById("doorselect").value === "C" ? 29 : 1;
        items.smallkey5 =
          document.getElementById("doorselect").value === "C" ? 29 : 3;
        items.smallkey6 =
          document.getElementById("doorselect").value === "C" ? 29 : 1;
        items.smallkey7 =
          document.getElementById("doorselect").value === "C" ? 29 : 2;
        items.smallkey8 =
          document.getElementById("doorselect").value === "C" ? 29 : 3;
        items.smallkey9 =
          document.getElementById("doorselect").value === "C" ? 29 : 4;
        items.smallkey10 =
          document.getElementById("doorselect").value === "C" ? 29 : 4;
        items.smallkeyhalf0 =
          document.getElementById("doorselect").value === "C" ? 29 : 1;
      ...
      if (
        !document.getElementById("shuffledkeys").checked &&
        !document.getElementById("shuffledbigkeys").checked &&
        flags.gametype != "R" &&
        document.getElementById("doorselect").value != "C"
      ) {
      ...
      switch (document.getElementById("goalselect").value) {
        ...
        case "F":
          if (flags.ganonvulncount === 8 || flags.goals === "A") {
            ...
            document
              .getElementById("ganondiv")
              .classList.add("ganon" + flags.ganonvulncount);
          }
          break;
        case "A":
    ...
    if (
      ...
      flags.overworldshuffle = document.getElementById("overworldbox").checked
        ? "Y"
        : "N";
    ...
    //Shopsanity
    if (
      document.getElementById("shopsanitybox").checked !=
      (flags.shopsanity != "N")
    ) {
      flags.shopsanity = document.getElementById("shopsanitybox").checked
        ? "Y"
        : "N";
    }
    ...
    if (flags.gametype === "S" && flags.entrancemode === "N") {
      ...
      if (flags.doorshuffle === "N") {
        document
          .getElementById("locationMap57")
          .classList.remove("unavailable");
        document
          .getElementById("locationMap63")
          .classList.remove("unavailable");
  ...
  window.updateLayout = function () {
    //Map layers
    document.getElementById("mapItemDiv").style.display =
      flags.entrancemode === "N" ? "block" : "none";
    document.getElementById("mapEntranceDiv").style.display =
      flags.entrancemode === "N" ? "none" : "block";

    //Hide HC and CT big keys if not needed
    document.getElementById("bigkeyhalf0").style.visibility =
      !flags.wildbigkeys || flags.doorshuffle != "C" ? "hidden" : "visible";
    document.getElementById("bigkeyhalf1").style.visibility =
      !flags.wildbigkeys || flags.doorshuffle != "C" ? "hidden" : "visible";

    //Hide HC and CT chests if neither Entrance nor Door Shuffle is on
    document.getElementById("agamagicsplitdiv").style.display =
      flags.entrancemode === "N" && flags.doorshuffle === "N"
        ? "none"
        : "block";
    document.getElementById("hcctchests").style.display =
      flags.entrancemode === "N" && flags.doorshuffle === "N"
        ? "none"
        : "block";
    document.getElementById("bighalfmagic").style.display =
      flags.entrancemode === "N" && flags.doorshuffle === "N"
        ? "block"
        : "none";
    document.getElementById("agasplitdiv").style.display =
      flags.entrancemode === "N" && flags.doorshuffle === "N"
        ? "block"
        : "none";
    if (
    ...
    if (flags.doorshuffle === "C") {
      ...
      for (var k = 0; k < 10; k++)
        document
          .getElementById("c" + k + "skdiv")
          .classList.remove("withcompass");
    ...
    if (flags.entrancemode === "N") {
      window.document.getElementById("locationMap1").style.visibility =
        "inherit";
    ...
    document.getElementById("locationMap55").style.visibility =
      flags.doorshuffle != "N" ? "hidden" : "visible";
    document.getElementById("locationMap57").style.visibility =
      flags.doorshuffle != "N" ? "hidden" : "visible";
    document.getElementById("locationMap58").style.visibility =
      flags.doorshuffle != "N" ? "hidden" : "visible";
    document.getElementById("locationMap63").style.visibility =
      flags.doorshuffle != "N" ? "hidden" : "visible";
    document.getElementById("locationMap65").style.visibility =
      flags.doorshuffle != "N" || (!flags.wildkeys && flags.gametype != "R")
        ? "hidden"
        : "visible";
    document.getElementById("locationMap66").style.visibility =
      flags.doorshuffle != "N" || (!flags.wildkeys && flags.gametype != "R")
        ? "hidden"
        : "visible";
    document.getElementById("bossMapAgahnim").style.visibility =
      flags.doorshuffle != "N" ? "hidden" : "visible";
    document.getElementById("castle").style.visibility =
      flags.doorshuffle != "N" ? "hidden" : "visible";
    document.getElementById("bossMap11").style.visibility =
      flags.doorshuffle != "N" ? "visible" : "hidden";
    document.getElementById("dungeon11").style.visibility =
      flags.doorshuffle != "N" ? "visible" : "hidden";
    document.getElementById("bossMap12").style.visibility =
      flags.doorshuffle != "N" ? "visible" : "hidden";
    document.getElementById("dungeon12").style.visibility =
      flags.doorshuffle != "N" ? "visible" : "hidden";

    //Hide shops outside of Shopsanity
    for (var k = 67; k < 79; k++) {
      document.getElementById("locationMap" + k).style.visibility =
        flags.shopsanity === "N" ? "hidden" : "visible";
    }
    document.getElementById("locationMap78").style.visibility = "hidden"; //Bomb Shop items not randomized yet

    document.getElementById("bombfloor").style.visibility =
      flags.doorshuffle != "C" ? "hidden" : "visible";

    document.getElementById("mirrorscroll").style.visibility =
      flags.doorshuffle === "N" ? "hidden" : "visible";

    document.getElementById("showpathsdiv").style.visibility =
      flags.doorshuffle === "N" && flags.overworldshuffle === "N"
        ? "hidden"
        : "visible";
  };
  ...
  window.updateLayoutTowers = function () {
    ...
    if (flags.entrancemode === "N") {
      if (
        flags.overworldshuffle === "N" ? flags.gametype === "I" : swapTowers
      ) {
        ...
        document.getElementById("bossMapAgahnim").style.top =
          flags.mapmode === "C" ? "5.5%" : "4.5%";
        document.getElementById("castle").style.left = "78%";
        document.getElementById("castle").style.top =
          flags.mapmode === "C" ? "5.5%" : "4.5%";
        ...
        document.getElementById("bossMap12").style.top =
          flags.mapmode === "C" ? "7.2%" : "5.5%";
        document.getElementById("dungeon12").style.left = "79.0%";
        document.getElementById("dungeon12").style.top =
          flags.mapmode === "C" ? "7.2%" : "5.5%";
      } else {
        ...
        document.getElementById("bossMap10").style.top =
          flags.mapmode === "C" ? "7.2%" : "5.5%";
        document.getElementById("dungeon10").style.left = "79.0%";
        document.getElementById("dungeon10").style.top =
          flags.mapmode === "C" ? "7.2%" : "5.5%";
  ...
  window.loadStyleAndChests = function () {
    ...
    document.getElementById("mainstyle").href =
      "css/" +
      (flags.entrancemode === "N" ? "" : "entrance") +
      "style.css?v=" +
      buildString;
    if (flags.mapmode === "C") {
      document.getElementById("maincompactstyle").href =
        "css/" +
        (flags.entrancemode === "N" ? "" : "entrance") +
        "smallmap.css?v=" +
        buildString;
  ...
  window.initializeSettings = function () {
    if (flags.opentower === "R") {
      ...
      document
        .getElementById("crystalsdiv")
        .classList.add("crystals" + flags.opentowercount);
    }
    ...
    document.getElementById("chest0").innerHTML =
      flags.doorshuffle === "C" ? items.chest0 - 1 + "+" : items.chest0;
    document.getElementById("chest1").innerHTML =
      flags.doorshuffle === "C" ? items.chest1 - 1 + "+" : items.chest1;
    document.getElementById("chest2").innerHTML =
      flags.doorshuffle === "C" ? items.chest2 - 1 + "+" : items.chest2;
    document.getElementById("chest3").innerHTML =
      flags.doorshuffle === "C" ? items.chest3 - 1 + "+" : items.chest3;
    document.getElementById("chest4").innerHTML =
      flags.doorshuffle === "C" ? items.chest4 - 1 + "+" : items.chest4;
    document.getElementById("chest5").innerHTML =
      flags.doorshuffle === "C" ? items.chest5 - 1 + "+" : items.chest5;
    document.getElementById("chest6").innerHTML =
      flags.doorshuffle === "C" ? items.chest6 - 1 + "+" : items.chest6;
    document.getElementById("chest7").innerHTML =
      flags.doorshuffle === "C" ? items.chest7 - 1 + "+" : items.chest7;
    document.getElementById("chest8").innerHTML =
      flags.doorshuffle === "C" ? items.chest8 - 1 + "+" : items.chest8;
    document.getElementById("chest9").innerHTML =
      flags.doorshuffle === "C" ? items.chest9 - 1 + "+" : items.chest9;
    document.getElementById("chest10").innerHTML =
      flags.doorshuffle === "C" ? items.chest10 - 1 + "+" : items.chest10;
    document.getElementById("chest11").innerHTML =
      flags.doorshuffle === "C" ? items.chest11 - 1 + "+" : items.chest11;
    document.getElementById("chest12").innerHTML =
      flags.doorshuffle === "C" ? items.chest12 - 1 + "+" : items.chest12;
    ...
    if (flags.mapmode != "N") {
      for (var k = 0; k < 79; k++) {
        document.getElementById("locationMap" + k).className =
          "location " +
          (k >= chests.length || chests[k].is_opened
            ? "opened"
            : chests[k].is_available());
      }
